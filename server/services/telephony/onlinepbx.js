 ...
 * disposition ANSWER/NOANSWER/CANCEL, waitsec/billsec). Один словарь на все
 * провайдеры: журнал, маршрут «звонок → заявка» и экран читают ЕГО, а не
...
export function normalizePbxCall(c) {
  ...
  return {
    ...
    internal_number: outbound ? caller : callee,
    waitsec: Math.max(0, duration - talk),
    billsec: talk,
    disposition: answered ? 'ANSWER' : (outbound ? 'CANCEL' : 'NOANSWER'),
    is_new_call: null,
    raw: c,
  };
}
