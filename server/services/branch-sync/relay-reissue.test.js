...
test('ответ без идентификатора НЕ ДОПИСЫВАЕТСЯ запрошенным', async () => {
  // Ревью 2026-09-03 (C1). Раньше сюда подставлялся тот идентификатор, о
  // котором просили, — и вызывающий, сверяя ответ с запросом, сверял бы запрос
  // сам с собой. Проверку, которую нельзя провалить, лучше не иметь вовсе, чем
  // иметь: она читается как защита, а защитой не является. Ответ, не назвавший
  // филиала, — это ответ, про который неизвестно, чей код в нём лежит.
  const dir = mainDir();
  for (const body of [{ enrollment_code: 'EM-1' }, { enrollment_code: 'EM-1', clinic_id: '' },
    { enrollment_code: 'EM-1', clinic_id: 42 }]) {
    const r = await reissueBranchOnControlPlane(dir, {
      clinicId: 'c-000005-b1', fetchImpl: reply(200, body), env: ENV,
    });
    assert.equal(r.ok, true, JSON.stringify(body));
    assert.equal(r.clinic_id, null, JSON.stringify(body));
  }
});
