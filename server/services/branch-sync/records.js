...
// ДОБАВЛЯЯ КОЛОНКУ В SHIPPED, проверьте её индекс и допишите её сюда. Если это
// колонка НОВОЙ таблицы — новой миграцией нужен и триггер, снимающий её строки
// из sync_minted при удалении (образец — 090): без него перечеканенный номер
// пережил бы сам документ и достался бы следующей строке с тем же uid.
...
function applyOne(db, rec, stats, ctx) {
  ...
  if (!waiting) {
    for (const [col, spec] of Object.entries(CODE_REFS[rec.tbl] || {})) {
      ...
      if (!code) continue;
      // МЯГКАЯ ССЫЛКА ДОГОНЯЕТ (CROSS_BRANCH_CALENDAR_V1). У существующей
      // строки колонку не трогают, если эту правку нам не адресовали (changed).
      // Исключение ровно одно и очень узкое: ссылка, которую мы НЕ СМОГЛИ
      // разрешить при первом приезде, и колонка с тех пор так и стоит пустой.
      //
      // Без него запись, обогнавшая справочник сотрудников, оставалась бы в
      // «Не назначено» НАВСЕГДА: отправитель врача с тех пор не правил, значит
      // в changed его нет и не будет, а у нас он NULL. Переписать здесь нечего
      // — колонка пуста, — поэтому это не «взять чужое поверх своего», а
      // достроить то, что мы сами приняли неполным. Намеренную местную очистку
      // это не отменяет: её защищает localWins ниже, а сама очистка уезжает
      // автору, и он перестаёт слать ссылку.
      const softCatchUp = () => {
        if (!spec.soft || id == null) return false;
        try { return ctx.q(`SELECT ${col} AS v FROM ${rec.tbl} WHERE id = ?`).get(id).v == null; }
        catch { return false; }
      };
      if (id != null && !wanted(col) && !softCatchUp()) continue;
      ...
      if (pid == null) {
        // CROSS_BRANCH_CALENDAR_V1 — МЯГКАЯ ССЫЛКА НА СПРАВОЧНИК: ключа у нас
        // нет, но строка всё равно заводится, БЕЗ этой колонки. Сегодня такая
        // одна — visits.doctor_id по логину (см. CODE_REFS в journal.js).
        //
        // Разница с услугой не стилистическая. Строка лабораторной очереди без
        // услуги — это работа, о которой неизвестно, что делать: ей правильно
        // ждать. Запись на приём без врача — это по-прежнему запись: время,
        // пациент и статус приехали, слот занят, и спрятать её от регистратуры
        // на том основании, что справочник сотрудников (отдельный канал, свой
        // такт) ещё не привёз человека, значило бы показать чужое здание
        // СВОБОДНЫМ там, где оно занято. А через 30 дней ожидания sync_pending
        // выселяет запись совсем — то есть «подождём» кончается «потеряли».
        //
        // Врач при этом не выдумывается: колонка остаётся пустой, запись видна
        // в дорожке «Не назначено», и следующая её правка (справочник к тому
        // времени доехал) проставит врача сама.
        if (spec.soft) continue;
        waiting = { tbl: spec.table, uid: code }; break;
      }
      cols.push(col); vals.push(pid);
    }
  }
  ...
  if (id == null) {
    ...
    //
    // ТЕПЕРЬ ЭТО «ГДЕ СДЕЛАНА РАБОТА», А НЕ «КТО ПРИВЁЗ» (ревью Фазы 3, NEW-1).
    // Раньше буква бралась из МЕТКИ, а метку чеканит собравший порцию: строка
    // здания C, приехавшая холодным засевом от здания R, садилась в базу как
    // «филиал R» — и подпись на карточке, и «Здание» в отчёте, и запрет правки
    // («изменить его можно только там») называли не тот дом. Настоящий дом
    // строки едет отдельным полем `home` (journal.js), которое отправитель
    // берёт из СВОЕГО sync_origin этой строки, — и попадает сюда как есть.
    //
    // МЕТКА ОСТАЁТСЯ ЗАПАСНЫМ ИСТОЧНИКОМ для соседа старой сборки, который
    // `home` не шлёт: у него всё как было. Метка при этом уже проверена isStamp
    // выше (^[0-9a-f]{12}-[0-9a-f]{4}-[A-Z]{1,8}$), а `home` проверяется той же
    // формой буквы (recordHome) — в колонку не попадёт ни мусор, ни имя.
    //
    // НАША СОБСТВЕННАЯ РАБОТА, ВЕРНУВШАЯСЯ К НАМ, — ЭТО NULL. Дом строки, равный
    // нашей букве, означает «выписано здесь»; так эту строку читают ВСЕ
    // потребители (record-origin.js: NULL — своё, буква — чужое; buildings.js;
    // `.is('sync_origin', null)` в кабинете врача, процедурах и лаборатории;
    // guardForeign в billing.js). Случай редкий — строку, которая у нас есть,
    // мы здесь не вставляем, — и означает он восстановление своего же после
    // потери; записать туда собственную букву значило бы объявить свои
    // документы чужими и запретить их править. Единственное место, которое от
    // этого чуть теряет, — пол засева (journal.js, floorStamp): у «своей» строки
    // он берётся из created_at, а у восстановленной created_at — это время
    // приёма. Цена ограничена этим редчайшим случаем и несравнима с тем, чтобы
    // объявить чужими все свои документы разом.
    const stamped = parseStamp(rec.stamp);
    const home = recordHome(rec);
    const origin = home
      ? (home === ctx.self ? null : home)
      : (stamped ? stamped.node : null);
    ctx.q(
 ...
 *
 * ЭТОГО ОКАЗАЛОСЬ МАЛО, И РЕШЕНИЕ ТЕПЕРЬ ХРАНИТСЯ (миграция 090, sync_minted;
 * ревью Фазы 3, NEW-2). Идемпотентность выше держалась на неподвижности
 * таблицы, а таблица движется: местный счёт, занимавший 'INV-26-00001',
 * аннулировали и удалили — и следующая же правка того же ЧУЖОГО счёта
 * возвращала ему 'INV-26-00001' вместо 'INV-26-00001-C'. Замерено ревью; то же
 * с номером карты.
 *
 * Номер карты и номер счёта — это то, что напечатано на квитанции, вписано в
 * бумажный журнал и по чему ищут в программе. Номер, который система один раз
 * показала, не имеет права смениться НИКОГДА — в том числе на «более
 * правильный». Поэтому первое приземление записывает свой ответ в sync_minted,
 * а КАЖДЫЙ следующий приезд той же (tbl, uid, col) берёт его дословно и не
 * пересчитывает ничего. Строку, которую перечеканивать не пришлось, здесь не
 * помнят вовсе; удалили строку — триггеры 090 унесли и решение.
 *
 * ИМЕННО ПОЭТОМУ ПОЗДНЯЯ, БОЛЕЕ ТОЧНАЯ БУКВА НОМЕР НЕ МЕНЯЕТ. Ниже
 * (remintLetter) буква берётся из настоящего дома строки, который едет по
 * проводу полем `home`; сосед старой сборки его не шлёт, и до его обновления
 * строка может лечь с буквой ПЕРЕСЫЛЬЩИКА. Когда сосед обновится и home
 * приедет, сохранённое решение всё равно победит: номер с неидеальной буквой,
 * который не менялся, лучше правильной буквы ценой переименования документа.
 *
 * СРАВНИВАЕТСЯ И ИСХОДНЫЙ НОМЕР (src). Сохранённый ответ отвечает на
 * конкретный вопрос — «что делать с приехавшим 'INV-26-00001'». Приедет ДРУГОЙ
 * исходный номер по той же строке — вопрос другой, и отвечать на него старым
 * ответом значило бы переименовать документ в чужой. Сегодня приложение номера
 * не правит вовсе (nextInvoiceNumber, триггер MRN в 080), так что ветка эта
 * теоретическая; молча полагаться на это — нет.
 *
...
function remintTakenKeys(ctx, rec, id, uniqueKeys, cols, vals, stats) {
  ...
  let letter;
  const kept = ctx.q('SELECT src, value FROM sync_minted WHERE tbl = ? AND uid = ? AND col = ?');
  const keep = ctx.q(`INSERT INTO sync_minted (tbl, uid, col, src, value) VALUES (?,?,?,?,?)
                      ON CONFLICT(tbl, uid, col) DO UPDATE SET
                        src = excluded.src, value = excluded.value,
  ...
  for (const col of uniqueKeys) {
    ...
    // РАНЬШЕ ЛЮБОЙ ПРОВЕРКИ ЗАНЯТОСТИ. Принятое решение не пересматривается ни
    // потому, что исходный номер вдруг освободился, ни потому, что приехала
    // буква получше: он уже на бумаге у людей.
    const was = kept.get(rec.tbl, rec.uid, col);
    if (was && was.src === value) {
      if (was.value !== value) vals[i] = was.value;
      continue;
    }
    ...
    vals[i] = minted;
    // Решение — В ТОЙ ЖЕ ТРАНЗАКЦИИ и в том же savepoint'е (applyGuarded), что
    // и сама строка: не встала строка — не должно остаться и памяти о номере,
    // которого никто не видел.
    keep.run(rec.tbl, rec.uid, col, value, minted);
...
/**
 * Буква здания, которой подписывается перечеканенный номер, — того, ГДЕ
 * ДОКУМЕНТ ВЫПИСАЛИ (ревью Фазы 3, NEW-1).
 *
 * Источники, от самого надёжного к запасному:
 *   1. строка уже здесь — её sync_origin: он проставлен один раз при вставке и
 *      переживает любую пересылку;
 *   2. поле `home` записи — настоящий дом строки, как его назвал отправитель
 *      (journal.js). Именно оно чинит сеть из трёх и более зданий: строку
 *      здания C, узнанную ЧЕРЕЗ здание R, раньше подписывали буквой R, потому
 *      что метку чеканит СОБРАВШИЙ ПОРЦИЮ, а не автор строки, — и один и тот же
 *      счёт носил в сети три разных номера;
 *   3. буква из метки — сосед старой сборки `home` не шлёт, и для него всё
...
/**
 * НАСТОЯЩИЙ ДОМ СТРОКИ, как его назвал отправитель (поле `home`, journal.js), —
 * или null, если он не назван или назван мусором.
 *
 * ЭТО НЕ rec.origin, и путать их нельзя. origin означает «кто прислал нам эту
 * порцию», и applyBatch сверяет его с тем соседом, чей блоб мы забрали: запись,
 * подписанная в этом блобе чужой буквой, не применяется вовсе. Два разных
 * вопроса — два разных поля: «через кого приехало» и «где сделана работа».
 */
function recordHome(rec) {
  const raw = rec && rec.home;
  if (typeof raw !== 'string' || !raw) return null;
  const up = raw.toUpperCase();
  return SELF_RE.test(up) ? up : null;
}
