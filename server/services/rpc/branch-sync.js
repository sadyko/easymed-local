...
// лекарство. Свои здесь — те, у которых лекарство ДРУГОЕ.
...
//
// «ЗАВЕДИТЕ ФИЛИАЛ ЗАНОВО» ОТСЮДА УБРАНО (ревью 2026-09-03). Совет выглядел
// бесплатным, а стоил двух необратимых вещей сразу: ещё одной буквы, которых
// конечное число и которые не возвращаются (letters.js), и ВТОРОЙ строки
// клиники у поставщика — то есть второй подписки в счёте владельца. Цену надо
// называть до нажатия, а не после.
REASONS.reissue_unknown_branch = 'Этот филиал заведён до обновления, и Easy-Med не знает его номер — перевыпуск ключа недоступен. Обратитесь в поддержку Easy-Med: заводить филиал заново не стоит — это тратит ещё одну букву и добавляет вторую клинику в счёт.';

// РЕВЬЮ 2026-09-03 (C1) — ОТВЕТ ПОСТАВЩИКА ДОКАЗЫВАЕТ АДРЕСАТА, а не
// подтверждает наши ожидания вежливым «ок».
//
// Перевыпуск адресуется номером филиала у поставщика, и у филиалов старого
// выпуска номер этот ВЫЧИСЛЕН (backfillBranchClinicIds), а не сохранён. Промах
// вычисления гасит работающий компьютер в другом здании и отдаёт филиалу ключ
// от чужой клиники — поэтому ответ сверяется дважды: по номеру и по ИМЕНИ. Не
// сошлось — не пишем НИЧЕГО.
REASONS.reissue_wrong_branch = 'Сервер Easy-Med перевыпустил ключ другого филиала — ключ не сохранён. Сообщите в поддержку Easy-Med.';
// СВОЕЙ ФРАЗЫ У НЕСОВПАВШЕГО ИМЕНИ ЗДЕСЬ НЕТ, и это решение (ревью 2026-09-03,
// второй круг): расхождение имён — не отказ, а РАЗГОВОР («Easy-Med знает этот
// филиал как …; вы его переименовали?»), и разговор этот идёт на экране, где у
// него есть словарь и обе кнопки. Отсюда наверх едут два имени и код
// reissue_name_mismatch, а слова к ним подбирает branch-sync-logic.js. Фраза
// сервера уехала бы в узбекскую клинику по-русски — известная дыра, и заводить
// в ней новую строку незачем.
...
/**
 * То же самое, но БЕЗ обновления метки времени у существующей строки.
 *
 * РЕВЬЮ 2026-09-03 (M9). У одной записи в control_state метка времени — не
 * бухгалтерия, а ДАННЫЕ: `branch_sync.enroll.<id>.updated_at` означает «когда
 * поставщик выдал этому филиалу номер», и по этому порядку восстанавливаются
 * номера филиалов старого выпуска (backfillBranchClinicIds). Перевыпуск
 * ПЕРЕЗАПИСЫВАЕТ код в той же строке — и обычный putState отправлял бы
 * перевыпущенный филиал в конец очереди, ломая порядок, которого он не менял.
 * Дальше это выглядело так: после первого же перевыпуска проверка якорями
 * переставала сходиться, и «нумерация филиалов не сходится с поставщиком»
 * печаталось в журнал при КАЖДОЙ загрузке списка — то есть предупреждение,
 * означавшее «у поставщика есть филиал, о котором мы не знаем», превращалось в
 * постоянный шум и переставало значить что-либо.
 */
function putStateKeepingTime(db, key, value) {
  db.prepare(`INSERT INTO control_state (key, value, updated_at)
              VALUES (?, ?, strftime('%Y-%m-%dT%H:%M:%SZ','now'))
              ON CONFLICT(key) DO UPDATE SET value = excluded.value`)
    .run(key, String(value));
}
function dropState(db, key) {
...
/**
 * ...и ОТКУДА этот номер взялся: сохранён при заведении или ВЫЧИСЛЕН.
 *
 * РЕВЬЮ 2026-09-03 (C1). Разница между «поставщик сам назвал этот номер» и «мы
 * вывели его из порядка заведения» — это разница между «перевыпускаем» и
 * «стреляем в чужое здание», и до этой пометки её негде было прочитать: обе
 * записи выглядели в control_state одинаково. Отдельным ключом, а не меткой
 * внутри значения, чтобы сам номер остался тем, чем был, и его читатели не
 * узнали ни о какой пометке.
 *
 * Стоит ровно на вычисленных: снимается она в одном месте — когда поставщик
 * СВОИМ ответом подтвердил и номер, и имя филиала (branchSyncReissueKey).
 */
const BRANCH_CLINIC_GUESS_PREFIX = 'branch_sync.clinic_guessed.';
const branchClinicGuessKey = (branchId) => BRANCH_CLINIC_GUESS_PREFIX + branchId;
 ...
 * единицу. Здесь стоит ПРОВЕРКА ЯКОРЯМИ: филиалы, чей номер сохранён при
 * заведении (то есть известен точно), обязаны совпасть с вычисленными.
 *
 * ЯКОРЕЙ МОЖЕТ НЕ БЫТЬ НИ ОДНОГО, и на клинике, которая обновляется в этот
 * выпуск, их не будет ВСЕГДА (ревью 2026-09-03, C1): сохранённые номера
 * появляются только у филиалов, заведённых ПОСЛЕ обновления, а восстанавливать
 * приходится как раз те, что заведены до. Проверка, которой нечего проверять,
 * не срабатывает никогда — то есть ровно там, где она нужна, её нет. Поэтому
 * догадка помечается ДОГАДКОЙ (branchClinicGuessKey), и настоящая защита стоит
 * дальше, в перевыпуске: у вычисленного номера он спрашивает владельца ДО сети
 * и сверяет имя филиала в ответе поставщика ПОСЛЕ. Здесь же остаётся то, что
 * здесь и можно: не гадать, когда известное с вычисленным разошлось.
 *
...
export function backfillBranchClinicIds(db, dataDir) {
  ...
  for (const p of plan) {
    ...
    try {
      // ПОМЕТКА ПЕРВОЙ, номер вторым, и порядок здесь — вся страховка. Номер
      // без пометки читается как сохранённый поставщиком, то есть как
      // доказанный, а он вычислен: перевыпуск перестал бы спрашивать ровно
      // там, где спросить обязан. Записались обе — хорошо; не записалась
      // пометка — не пишется и номер, и филиал остаётся «номер неизвестен»
      // (честный отказ); не записался номер — остаётся пометка без номера,
      // которая не значит ничего и никому не мешает.
      putState(db, branchClinicGuessKey(p.id), '1');
      putState(db, branchClinicKey(p.id), p.guess);
      filled += 1;
 ...
 * заведён».
 *
 * `renew: true` — ВЫПИСАТЬ НОВУЮ ПОВЕРХ СТАРОЙ (перевыпуск ключа филиала).
 *
 * РЕВЬЮ 2026-09-03 (I3): раньше перевыпуск делал это сам и в другом порядке —
 * стирал сохранённую учётку, а потом звал выписку. Выписка ходит в сеть, а
 * клиника офлайновая по построению: любой неудачный поход оставлял филиал
 * ВОВСЕ БЕЗ учётки (второй раз её не покажут — она невоспроизводима), и ключ
 * уезжал с relay_token: null, то есть без резервного канала и молча. Здесь
 * стирания нет ни на каком пути: старое значение перезаписывается ТОЛЬКО
 * успешной выпиской. Не вышло — у филиала остаётся прежняя рабочая учётка,
 * а экран говорит, что перевыпустить её не удалось.
 */
async function ensureBranchToken(db, dataDir, branchId, mintImpl, { renew = false } = {}) {
  ...
  if (existing && !renew) return { ok: true };

  // BRANCH_RECORDS_V1 (Задача 7a) — буквы узлов группы ДЛЯ ОБЛАСТИ токена.
  //
  // ДОПОЛНЕНИЕ, А НЕ СПИСОК: relayScope выписывает весь алфавит A..Z авансом —
  // иначе филиал, заведённый ПОЗЖЕ выдачи чужого ключа, остался бы невидим для
  // соседа НАВСЕГДА: выписка не повторяется, а ключ собирается из СОХРАНЁННОГО
  // токена (branchKeyFor выше). Переданные буквы только ДОПОЛНЯЮТ алфавит и
  // нужны для сетей больше 26 зданий, где letters.js выдаёт AA и дальше: такие узлы
  // алфавитом не покрыты, а знает о них только эта сторона (branches ведёт главная
  // клиника). Сузить область они не могут.
  //
 ...
 *
 * ЧТО ИМЕННО ПЕРЕСТАЁТ РАБОТАТЬ У СТАРОГО КОМПЬЮТЕРА, дословно (ревью
 * 2026-09-03, I2 — прежний комментарий обещал здесь больше, чем происходит):
 *
 *   • CHECK-IN — ДА, СРАЗУ. Поставщик гасит install_token прежней установки, и
 *     она с этой секунды не подтверждает лицензию;
 *   • ДОСТУП К ОБМЕНУ ДАННЫМИ — НЕТ. Учётка резервного канала выписывается
 *     НОВАЯ, но старую никто не отзывает: ручка выписки у поставщика
 *     аддитивна, ручки отзыва в этой задаче не заводится, и старый токен живёт
 *     до уборки по сроку — pruneRelayTokens (control-plane
 *     routes/relay-token.js) сметает лишь то, чем не пользовались 30 дней.
 *     Значит старая установка, пока её включают, продолжает и ЧИТАТЬ, и
 *     ПИСАТЬ блобы группы: ключ группы у неё тот же, он в ключе подключения.
 *     Отобрать это сразу можно одним способом — перевыпустить ключ
 *     синхронизации («Дополнительно»): у группы меняются и идентификатор, и
 *     ключ шифрования, и старая копия перестаёт и попадать по адресу, и
 *     расшифровывать. Экран говорит владельцу ровно это, теми же словами
 *     (BRANCH_KEY_REISSUE_WARNING).
 *
 ...
 *
 * КОМУ АДРЕСОВАН ПЕРЕВЫПУСК — ГЛАВНЫЙ ВОПРОС ЭТОЙ ФУНКЦИИ (ревью 2026-09-03,
 * C1). Адресат — номер филиала у поставщика, и он бывает двух сортов: СОХРАНЁН
 * (поставщик назвал его сам, отвечая на заведение) и ВЫЧИСЛЕН
 * (backfillBranchClinicIds вывел его из порядка заведения — так у всех
 * филиалов, заведённых до этого выпуска). Вычисленный может промахнуться:
 * достаточно одного ребёнка, которого поставщик завёл, а мы не записали, —
 * и все следующие номера съезжают на единицу. Цена промаха — погашенный
 * install_token РАБОТАЮЩЕГО компьютера в другом здании и ключ от чужой клиники
 * в руках у филиала. Поэтому:
 *
 *   1. НОМЕР В ОТВЕТЕ СВЕРЯЕТСЯ ВСЕГДА. Разошёлся с запрошенным (или его нет
 *      вовсе) — не пишем ничего, reissue_wrong_branch. Обойти это нельзя ничем:
 *      записать код, про который неизвестно, чей он, нельзя ни по какому
 *      подтверждению;
 *   2. у ВЫЧИСЛЕННОГО номера сначала спрашивают владельца — до сети, отдельным
 *      вопросом, с номером и именем на экране (reissue_confirm, args.confirm);
 *   3. ...и ТОЛЬКО у вычисленного сверяется ИМЯ филиала в ответе. Не сошлось —
 *      не пишем ничего и возвращаем оба имени: экран спрашивает владельца, не
 *      он ли переименовал филиал здесь, и его «да» приезжает как
 *      args.accept_name;
 *   4. подтверждённый — именем поставщика или владельцем — номер перестаёт быть
 *      вычисленным: пометка снимается, и второй раз никто ничего не спрашивает.
 *
 * ПОЧЕМУ ИМЯ НЕ СВЕРЯЕТСЯ У СОХРАНЁННОГО НОМЕРА (ревью 2026-09-03, второй
 * круг). Сохранённый номер записан из ответа поставщика на заведение ИМЕННО
 * ЭТОЙ строки — совпадение clinic_id уже и есть доказательство адресата,
 * добавить к нему имя нечего. А расходиться имена будут постоянно: филиал
 * переименовывают в «Настройки → Филиалы», и поставщику это переименование не
 * уезжает никуда. Проверка имени здесь ломала бы перевыпуск переименованному
 * филиалу НАВСЕГДА — каждая попытка гасит код у поставщика, а записать новый мы
 * отказываемся, — и лечилось бы это звонком в поддержку из-за обычного
 * переименования. Плохой размен: страховка без риска против поломки без выхода.
 *
 * ПОРЯДОК «ЗАПРОС → ПРОВЕРКА → ЗАПИСЬ» ИНАЧЕ НЕ СТРОИТСЯ: имя филиала у
 * поставщика узнаётся только из ответа, а ответ приходит после того, как
 * поставщик уже погасил старый код. Значит проверка имени спасает не старый
 * код (он сгорел), а СЛЕДУЮЩИЙ ШАГ: она не даёт записать чужой код себе и
 * отдать филиалу ключ от чужой клиники. Единственная защита ДО выстрела —
 * вопрос владельцу, пункт 2.
 */
export async function branchSyncReissueKey(db, args, user, {
  ...
  // ВЫЧИСЛЕННЫЙ НОМЕР СПРАШИВАЕТ ВТОРОЙ РАЗ, и спрашивает ДО СЕТИ — это
  // единственный момент, когда чужой компьютер ещё можно не гасить (ревью
  // 2026-09-03, C1). Не отказ: владелец, который знает свои филиалы, отвечает
  // «да» и получает то, за чем пришёл; владелец, у которого номер съехал,
  // видит на экране «-b2» против третьего филиала и звонит в поддержку.
  //
  // ok:false, а не исключение: это не поломка, а незаданный вопрос, и экран
  // обязан отличать одно от другого — на исключении он печатает красную
  // строку, а здесь показывает второе окно.
  const guessed = !!getState(db, branchClinicGuessKey(branch.id));
  // accept_name — ответ на ВТОРОЙ разговор об имени (ниже), и он тоже означает
  // «я уже подтвердил этот филиал»: попадают сюда только после первого окна.
  const acceptName = !!(args && args.accept_name === true);
  if (guessed && !acceptName && !(args && args.confirm === true)) {
    return { ok: false, reason: 'reissue_confirm', vendor_id: clinicId, branch_name: branch.name };
  }

  const r = await reissueImpl(dataDir, { clinicId });
  if (!r.ok) throw new RpcError(reasonText(r.reason), 400);

  // --- ОТВЕТ ДОКАЗЫВАЕТ АДРЕСАТА -------------------------------------------
  // Обе проверки стоят ДО единственной записи в этой функции: ошибиться здесь
  // значит записать себе код от ЧУЖОЙ клиники и отдать его филиалу ключом.
  if (r.clinic_id !== clinicId) {
    // Номер в ответе — это то, что поставщик считает перевыпущенным. Разошёлся
    // с тем, о чём просили (или его нет вовсе) — мы не знаем, чей код держим
    // в руках, и записывать его нельзя ни при каких обстоятельствах.
    console.warn('[branch-sync] the vendor reissued', r.clinic_id, 'but we asked for', clinicId,
      '— storing nothing');
    throw new RpcError(reasonText('reissue_wrong_branch'), 400);
  }
  // ...А ВОТ ИМЯ ПРОВЕРЯЕТСЯ НЕ ВЕЗДЕ, и это правка ревью 2026-09-03 (второй
  // круг). Имя — ЕДИНСТВЕННОЕ, что поставщик знает о филиале независимо от нас,
  // и потому единственное, чем можно поймать съехавший номер: номер в ответе он
  // повторит за нами всегда. Но проверка эта нужна ровно там, где номер
  // ВЫЧИСЛЕН, и вредна там, где он сохранён:
  //
  //   • НОМЕР СОХРАНЁН — совпадение clinic_id УЖЕ доказательство. Номер записан
  //     из ответа поставщика на заведение ИМЕННО ЭТОЙ строки, промахнуться
  //     нечем. А имена разойтись могут запросто: филиал переименовывают в
  //     «Настройки → Филиалы» (обычное дело), и поставщику переименование не
  //     уезжает никуда. Отказать здесь значило бы ломать перевыпуск каждому
  //     переименованному филиалу НАВСЕГДА: код у поставщика гаснет при каждой
  //     попытке, а записать новый мы отказываемся. Поэтому ключ пишется, а
  //     расхождение уходит в журнал — там оно и нужно;
  //   • НОМЕР ВЫЧИСЛЕН — доказательства нет никакого, и имя остаётся
  //     единственным. Здесь отказ остаётся, но НЕ тупиком: экран показывает оба
  //     имени и спрашивает владельца, не он ли переименовал филиал. Ответ «это
  //     тот же филиал» приходит сюда как accept_name.
  const vendorName = typeof r.name === 'string' ? r.name.trim() : '';
  const nameMatches = !!vendorName && sameBranchName(vendorName, branch.name);
  if (vendorName && !nameMatches) {
    if (!guessed) {
      console.warn('[branch-sync] the vendor still calls branch', branch.id, JSON.stringify(vendorName),
        'while it is now', JSON.stringify(branch.name || ''), '— renamed here, storing the key anyway');
    } else if (!acceptName) {
      // ok:false, а не исключение: это снова ВОПРОС, а не поломка. Оба имени
      // едут наверх — экран из них и собирает разговор, свой на каждом языке
      // (фразы сервера через словарь не проходят).
      console.warn('[branch-sync] the vendor calls the guessed id for branch', branch.id,
        'something else — storing nothing until the owner confirms');
      return {
        ok: false, reason: 'reissue_name_mismatch',
        vendor_name: vendorName, local_name: branch.name || '',
      };
    }
  }
  ...
  try {
    ...
    //
    // putStateKeepingTime, А НЕ putState (ревью 2026-09-03, M9): метка времени
    // этой строки — не «когда мы её трогали», а «когда поставщик выдал номер»,
    // и по ней восстанавливается порядок филиалов старого выпуска. Перевыпуск
    // порядка не менял, и двигать себя в конец очереди ему незачем.
    putStateKeepingTime(db, branchEnrollKey(branch.id), r.enrollment_code);
  } catch (e) {
  ...
  // ВЫЧИСЛЕННЫЙ НОМЕР, ПОДТВЕРЖДЁННЫЙ, перестаёт быть вычисленным: второй раз
  // владельца об одном и том же не спрашивают. Подтверждений ровно два, и оба
  // настоящие: имя сошлось у поставщика (nameMatches) — или владелец, увидев
  // оба имени, сказал «это тот же филиал» (acceptName). Один лишь повторённый
  // за нами номер не доказывает ничего, и пустое имя в ответе — тоже.
  //
  // ОТДЕЛЬНЫМ try, ПОСЛЕ записи кода, и это тот же размен, что в
  // ensureBranchEnroll: код — то, ради чего сюда шли, и объявлять успешный
  // перевыпуск неудачным из-за неснятой пометки нельзя. Пометка осталась —
  // владельца спросят ещё раз, только и всего.
  if (guessed && (nameMatches || acceptName)) {
    try { dropState(db, branchClinicGuessKey(branch.id)); }
    catch (e) { console.warn('[branch-sync] could not clear the guessed-id mark:', e && e.message); }
  }

  // Учётка резервного канала — заново, и ВЫПИСКА ИДЁТ ПЕРВОЙ (ревью
  // 2026-09-03, I3): renew перезаписывает сохранённую учётку только успешным
  // ответом поставщика. Прежний порядок — стереть, потом сходить в сеть —
  // оставлял офлайновую клинику вовсе без учётки и отдавал ключ с пустым
  // резервным каналом.
  const relay = await ensureBranchToken(db, dataDir, branch.id, mintImpl, { renew: true });
...
/**
 * ОДНО ЛИ ЭТО ИМЯ ФИЛИАЛА — по-человечески, а не побайтово (ревью 2026-09-03).
 *
 * Сверяются имя, которое помнит поставщик, и имя из строки списка. Требовать
 * побайтового совпадения нельзя: имя вводит человек, и «Чиланзар » с хвостовым
 * пробелом, «чиланзар» строчными, «Мед-Центр» против «Мед Центр» и кавычки
 * разного начертания — это одно и то же имя, а не разные филиалы. Ложное
 * несовпадение здесь стоит дорого: код у поставщика уже погашен, и отказ
 * оставляет филиал без ключа.
 *
 * Что остаётся различимым — буквы и цифры: «Филиал 1» и «Филиал 2» не
 * схлопнутся, а именно на них проверка и рассчитана.
 */
function normBranchName(s) {
  return String(s == null ? '' : s)
    .normalize('NFC')
    .toLowerCase()
    // «ё» и «е» в русских названиях пишут вперемешку — в том числе один и тот
    // же человек в двух полях одного экрана.
    .replace(/ё/g, 'е')
    // Всё, что не буква и не цифра, — разделитель: пробелы, дефисы, кавычки
    // любого начертания, точки. Схлопывается заодно и повторный пробел.
    .replace(/[^\p{L}\p{N}]+/gu, ' ')
    .trim();
}
const sameBranchName = (a, b) => normBranchName(a) === normBranchName(b);
