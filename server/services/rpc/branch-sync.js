...
// Обе дырки заполняются именами (fillReason ниже): без них фраза не отвечает на
// единственный вопрос, который у владельца после неё возникнет, — «а какой же
// филиал я тогда перевыпустил». Последнее предложение называет цену честно:
// код у поставщика уже погашен, и старый ключ мёртв независимо от того,
// записали мы новый или нет.
REASONS.reissue_name_mismatch = 'Сервер Easy-Med считает, что этот ключ принадлежит филиалу «{vendor}», а не «{local}». Ключ не перевыпущен — сообщите в поддержку Easy-Med. Старый ключ этого филиала больше не действует.';
...
function reasonVars(vars) {
  ...
  // РЕВЬЮ 2026-09-03 (C1) — два имени филиала в фразе о несовпадении: то, что
  // считает поставщик, и то, что записано здесь. Фраза без них («ключ
  // принадлежит другому филиалу») не говорит владельцу ни какому, ни что
  // проверять, — то есть ведёт в поддержку с пустыми руками.
  for (const key of ['vendor', 'local']) {
    if (typeof vars[key] === 'string' && vars[key]) out[key] = vars[key];
  }
  return out;
}
 ...
 *
 *   1. у ВЫЧИСЛЕННОГО номера сначала спрашивают владельца — до сети, отдельным
 *      вопросом, с номером и именем на экране (reissue_confirm, args.confirm);
 *   2. ответ поставщика ДОКАЗЫВАЕТ адресата: сверяется и номер, и ИМЯ филиала.
 *      Не сошлось — не пишем НИЧЕГО и говорим об этом громко;
 *   3. сойдясь по имени, вычисленный номер перестаёт быть вычисленным: пометка
 *      снимается, и второй раз владельца не спрашивают.
 *
...
export async function branchSyncReissueKey(db, args, user, {
  ...
  if (guessed && !(args && args.confirm === true)) {
  ...
  if (vendorName && !sameBranchName(vendorName, branch.name)) {
    // ИМЯ — ЕДИНСТВЕННОЕ, ЧТО ПОСТАВЩИК ЗНАЕТ О ФИЛИАЛЕ НЕЗАВИСИМО ОТ НАС, и
    // потому единственная проверка, способная поймать съехавший номер: номер в
    // ответе он повторит за нами всегда.
    //
    // Лечится это владельцем, а не программой, поэтому фраза называет ОБА
    // имени: увидев «Юнусабад» там, где ждал «Себзар», он поймёт, что номера
    // съехали. Если же имена разошлись потому, что филиал переименовали здесь
    // (список филиалов это позволяет, а поставщику переименование не уезжает),
    // то в фразе стоит имя, которое поставщик помнит, — и вернуть его на место
    // владелец может сам.
    console.warn('[branch-sync] the vendor calls branch', branch.id, 'something else — storing nothing');
    throw new RpcError(
      reasonText('reissue_name_mismatch', { vendor: vendorName, local: branch.name || '' }), 400,
    );
  }
  ...
  // ВЫЧИСЛЕННЫЙ НОМЕР, ПОДТВЕРЖДЁННЫЙ ИМЕНЕМ, перестаёт быть вычисленным:
  // второй раз владельца об одном и том же не спрашивают. Только при совпавшем
  // имени — один лишь повторённый за нами номер не доказывает ничего.
  //
