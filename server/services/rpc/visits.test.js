...
test('ensure_visit: one visit per patient per day — same day reuses, next day creates', () => {
  const db = freshDb();
  const a = ensureVisit(db, { patient_id: 1, date: '2026-08-07T09:00:00Z', doctor_id: 2 }, REG);
  assert.equal(a.created, true);
  assert.equal(a.visit.visit_date, '2026-08-07T09:00:00Z');

  // afternoon service, SAME day -> same visit, no new row
  const b = ensureVisit(db, { patient_id: 1, date: '2026-08-07T16:30:00Z' }, REG);
  assert.equal(b.created, false);
  assert.equal(b.visit.id, a.visit.id);

  // next day -> its own visit
  const c = ensureVisit(db, { patient_id: 1, date: '2026-08-08T10:00:00Z' }, REG);
...
test('ensure_visit: bare date works; doctor backfills onto a doctor-less day', () => {
  const db = freshDb();
  const a = ensureVisit(db, { patient_id: 1, date: '2026-08-09' }, REG);
  assert.equal(a.created, true);
  assert.equal(a.visit.doctor_id, null);
  const b = ensureVisit(db, { patient_id: 1, date: '2026-08-09T12:00:00Z', doctor_id: 2 }, REG);
  assert.equal(b.created, false);
  assert.equal(b.visit.doctor_id, 2);   // first assigned doctor lands on the day visit
});

test('ensure_visit: cancelled day does not swallow a new booking', () => {
  const db = freshDb();
  const a = ensureVisit(db, { patient_id: 1, date: '2026-08-10T09:00:00Z' }, REG);
  db.prepare("UPDATE visits SET status='cancelled' WHERE id=?").run(a.visit.id);
  const b = ensureVisit(db, { patient_id: 1, date: '2026-08-10T11:00:00Z' }, REG);
  assert.equal(b.created, true);
  assert.notEqual(b.visit.id, a.visit.id);
});

test('ensure_visit: validation + roles', () => {
  const db = freshDb();
  assert.throws(() => ensureVisit(db, { patient_id: 999, date: '2026-08-07' }, REG), /patient not found/);
  assert.throws(() => ensureVisit(db, { patient_id: 1, date: 'nope' }, REG), /date must be ISO/);
  assert.throws(() => ensureVisit(db, { patient_id: 1, date: '2026-08-07' }, { id: 9, role: 'cashier' }), /not allowed/);
});

test('CRM_AUTO_CAME_V1: ensure_visit flips linked active CRM leads to came, leaves others alone', () => {
  const db = freshDb();
  ...
  ensureVisit(db, { patient_id: 1, date: '2026-08-09' }, REG);
  ...
  ensureVisit(db, { patient_id: 2, date: '2026-08-09' }, REG);
...
test('CRM_FUTURE_LEAD_V2: a visit closes today\'s and overdue leads, never a future booking', () => {
  const db = freshDb();
  ...
  ensureVisit(db, { patient_id: 1, date: '2026-09-15' }, REG);
