...
import { updateMyDoctorProfile } from './doctor-profile.js';   // RPC_PORT_V1
...
export const RPC = {
  ...
  // STOCK_REQUEST_V1 — заявка себе или своему отделу; минимум и норма на руках
  // (себе — сам, отделу — заведующая, любому — склад и администратор);
  // остаток ниже минимума подаёт заявку сам (rpc/stock-requests.js).
  // stock_minimums_list — чтение (READ_ONLY_RPCS в control/gate.js).
  stock_request_create:          (db, args, user) => stockRequestCreate(db, args, user),
  ...
  // RPC_PORT_V1 — кабинет врача → «Мой профиль»: своя строка, белый список
  // колонок (rpc/doctor-profile.js). Пишет — в READ_ONLY_RPCS его нет.
  update_my_doctor_profile:       (db, args, user) => updateMyDoctorProfile(db, args, user),
  ...
  // LIS_INGEST_V1 — Лаборатория → «Анализаторы». Сами устройства идут обычным
  // /api/db (они объявлены в реестре схемы); здесь только то, чего таблицей не
  ...
  // CALL_RECORDING_V1 — ссылка на запись ЭТОГО разговора, по требованию.
  telephony_call_recording:  (db, args, user) => telephonyCallRecording(db, args, user),
