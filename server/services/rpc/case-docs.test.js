...
// ─── ЛОВУШКА ЧАСА ЗАПУСКА (CASE_DOCS_LOCAL_DAY_V1, 2026-09-20) ──────────────
//
// Этот файл отсчитывал всё от `Date.now()`, и тест дневника падал ПО НОЧАМ:
// в 23:10 и в 00:30 «вчерашние сутки закрыты записью за вчера» давало
// `overdue` вместо `pending`, а днём и в CI (UTC) тот же тест проходил.
// Виноват был не часовой пояс сам по себе, а ДВЕ РАЗНЫЕ ШКАЛЫ в одном тесте:
//
//   • СУТКИ ДНЕВНИКА СЕРВЕР НАРЕЗАЕТ ОТ КОЙКИ, а не от календаря: окно w — это
//     [admitted_at + 24w, admitted_at + 24w + 24) (`periodState`). Тест клал
//     пациента «30 часов назад» от часа запуска — и граница окон ехала вместе
//     с этим часом;
//   • НАЗВАННЫЙ ВРАЧОМ ДЕНЬ СТОИТ НА МЕСТЕ: `entry_date` сервер ставит в
//     ПОЛДЕНЬ UTC этой даты (`stampOf`, «Полдень взят намеренно»).
//
// Полдень «вчера» попадал в первое окно, только пока UTC-час запуска ≤ 18.
// При UTC 19:58 (= 00:58 в Ташкенте, UTC+5) он оказывался на 1.98 ч РАНЬШЕ
// самого поступления: первое окно оставалось незакрытым, дневник —
// «просроченным». То же и со второй записью: «сегодня» падало в первые сутки
// вместо текущих. В пересчёте на местное время UTC+5 это окно поломки —
// с 23:00 до 05:00; в UTC-машине CI — с 18:00 до полуночи, просто туда ещё
// не попадали.
//
// Поэтому опорный момент теста ЗАКРЕПЛЁН (последний наступивший полдень UTC),
// а не взят с часов. Мерка дней здесь — та же, что у сервера: UTC. Местного
// календаря в этом коде нет вовсе (ни одного `date(..., 'localtime')`:
// сутки дневника не календарные, они отсчитываются от койки), поэтому считать
// ожидания по местному дню было бы не «как сервер», а другой сдвиг тех же
// граблей — с ним тест ломался бы в UTC-полночь CI.
const H = 3600 * 1000;
/**
 * Момент, от которого отсчитывает весь файл. РОВНО ПОЛДЕНЬ UTC — потому что
 * сервер ставит названный день в 12:00Z: и «вчера», и «сегодня» ложатся тогда
 * в свои сутки с запасом в шесть часов от границы, при любом часе запуска и в
 * любом поясе. «Последний НАСТУПИВШИЙ» — чтобы посев не оказался в будущем.
 */
const NOW = (() => {
  const noon = new Date();
  noon.setUTCHours(12, 0, 0, 0);
  return noon.getTime() <= Date.now() ? noon.getTime() : noon.getTime() - 24 * H;
})();
const iso = (ms) => new Date(ms).toISOString().replace(/\.\d{3}Z$/, 'Z');
/** «столько-то часов назад / вперёд от опорного момента». */
const at = (hours) => iso(NOW + hours * H);
/** День записи ТОЙ ЖЕ меркой, какой его читает сервер: `stampOf` → 12:00Z. */
...
// NEXT_STEP_OVERDUE_V1 — когда просрочено ВСЁ, указатель всё равно указывает.
//
// Владелец прислал снимок обзора: «Все документы оформлены» — и строкой ниже
// «оформлено 0 из 8 · просрочено 7». Причина была здесь: выполнимых пунктов
// не осталось, next_kind оказывался пустым, и экран читал пустоту как
// готовность. Указатель и состояние разведены: next_kind называет самый
// ранний просроченный, а состояние пункта остаётся «просрочен».
test('NEXT_STEP_OVERDUE_V1: всё открытое просрочено — «следующий шаг» называет самый ранний просроченный, а состояния не переписываются', () => {
  const ctx = seed();
  const adm = inBed(ctx, 30);
  // Через 400 часов после размещения все сроки по часам прошли.
  const st = docs(ctx, adm, 400);

  assert.ok(st.items.some((i) => i.state === 'overdue'), 'к этому часу просроченные обязаны быть');
  assert.equal(st.items.filter((i) => i.state === 'next').length, 0,
      'выполнимых пунктов не осталось — заметной кнопки в списке быть не должно');
  assert.ok(st.next_kind, 'указатель «следующий шаг» пуст при непустом списке недооформленного');

  const pointed = itemOf(st, st.next_kind);
  assert.equal(pointed.state, 'overdue', 'состояние пункта переписано ради указателя: ' + pointed.state);
  assert.notEqual(st.next_kind, 'discharge', 'выписной эпикриз лежащему пациенту не предлагают и просроченным');

  // Самый ранний по регламенту, а не любой.
  // Титульный лист из указателя исключён по прежнему правилу: «следующий
  // шаг» — врачебный, а лист заполняет медсестра.
  const firstOverdue = st.items.find((i) => i.required && i.state === 'overdue'
    && i.due_rule !== 'at_discharge' && i.kind !== 'title');
  assert.equal(st.next_kind, firstOverdue.kind, 'указатель показывает не на самый ранний просроченный');

  // Обзор в этом состоянии не имеет права говорить «все оформлены».
  assert.ok(st.progress.overdue > 0, 'счётчик просроченного пуст');
  assert.ok(st.progress.done < st.progress.total, 'комплект не полон, а прогресс говорит обратное');
  ctx.db.close();
});
