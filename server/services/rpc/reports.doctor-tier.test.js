...
// считается: по счёту возврат.
test('возвращённый счёт не считается', () => {
  const c = clinic();
  c.line({ day: '2026-09-03', status: 'completed', invoiceStatus: 'refunded' });
  c.lines(25);
  assert.equal(fee(c.db), 25 * 30000);
});

// Ревью C1 — отмена СЧЁТА работу не отменяет: выполненная строка отменённого
// счёта считается и платится как невыставленная, и номер ступени у неё есть.
test('выполненная строка отменённого счёта считается ступенью', () => {
  const c = clinic();
