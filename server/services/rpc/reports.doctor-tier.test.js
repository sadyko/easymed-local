...
function clinic({ pct = 30, tierFrom = 25, tierPct = 40, fix = null, taxRate = 0 } = {}) {
  ...
  const line = ({ day = '2026-09-05', at = null, status = 'added', paid = true, invoiceStatus = 'paid', qty = 1, price = 100000 } = {}) => {
    const id = ++seq;
...
//
// Заодно тест закрепляет, что у зарплатного отчёта и у нумерации РАЗНЫЕ даты:
// отчёт платит по дате СЧЁТА (i.created_at), нумерация считает по дате ВИЗИТА.
// Пациент заплатил днём 30.09 за ночной приём — строка оплачена в сентябре, но
// по ступени это первая октябрьская, и идёт она по базовому проценту.
test('граница месяца — по местному времени клиники: 30.09 20:30Z — это уже октябрь при UTC+5', (t) => {
  ...
  const boundary = visitYm === '2026-09' ? 40000 : 30000;
  if (new Date().getTimezoneOffset() === -300) {
    assert.equal(boundary, 30000,
      'на UTC+5 ночной приём 01:30 01.10 обязан быть первым ОКТЯБРЬСКИМ, а не 26-м сентябрьским');
  }
  assert.equal(fee(c.db), 25 * 30000 + boundary,
    'месяц ступени посчитан не по местному времени: визит попал в ' + visitYm);

  // Оплата при этом сентябрьская: в октябрьском отчёте этой строки нет вовсе.
  assert.equal(fee(c.db, { from: '2026-10-01', to: '2026-10-31' }), 0,
    'строка уехала в октябрьскую ведомость — отчёт платит по дате счёта');
});
