...
//
// Журнал был экраном администратора и кладовщика, и «почём куплено» там было
// уместно. Область видимости («своё / свой отдел / вся клиника») открыла его
// медсестре и заведующей — а закупочная цена оставалась в КАЖДОЙ строке
// ответа. Экран её не рисовал (колонки у этих областей нет), но это разметка,
// а не защита: ответ сервера виден во вкладке «Сеть» браузера одним движением.
//
// Поэтому проверяется ПОЛЕ ОТВЕТА, а не колонка таблицы: право видеть строку и
// право видеть её цену — разные права, и решает их сервер.
// ───────────────────────────────────────────────────────────────────────────

test('закупочная цена приходит только тому, кто видит всю клинику — остальным её НЕТ В ОТВЕТЕ', () => {
  const db = seed();
  try {
    busyDay(db);
    const admin = stockMovementsList(db, {}, ADMIN);
    assert.equal(byKind(admin, 'receive').find((m) => m.product_id === 7).unit_cost, 20000,
        'администратор потерял закупочную цену — её убирают у отдела, а не у всех');
    assert.equal(byKind(admin, 'issue').some((m) => m.unit_cost != null), true,
        'подготовка: у выдачи записана себестоимость (иначе проверять нечего)');
    assert.equal(byKind(stockMovementsList(db, {}, INV), 'receive').find((m) => m.product_id === 7).unit_cost, 20000,
        'кладовщик видит всю клинику — и её закупочные цены');

    for (const [who, user] of [['заведующая', HEAD], ['медсестра', NURSE]]) {
      const r = stockMovementsList(db, {}, user);
      assert.ok(r.movements.length, 'подготовка: ' + who + ' должна видеть свои движения');
      assert.equal(r.movements.every((m) => m.unit_cost === null), true,
          who + ' читает закупочную цену прямо из ответа сервера: '
          + JSON.stringify(r.movements.map((m) => [m.product_name, m.unit_cost])));
      // Цену сняли, а не обрезали ответ: строка осталась целой.
      assert.equal(r.movements.every((m) => m.product_name && m.qty !== undefined && m.unit), true,
          'вместе с ценой из строки пропало что-то ещё');
    }
    // И через сужение «Моих запасов» цена тоже не просачивается.
    const mine = stockMovementsList(db, { kind: 'issue', only: 'to_me' }, NURSE);
    assert.equal(mine.movements.length, 1, 'подготовка: медсестре что-то выдали');
    assert.equal(mine.movements[0].unit_cost, null, '«Мои запасы» показывают медсестре закупочную цену');
  } finally { db.close(); }
});
