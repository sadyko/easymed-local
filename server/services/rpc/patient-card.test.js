...
// ===========================================================================
// TELEGRAM_PATIENT_BADGE_V1 — состояние Telegram едет вместе с картой
// ===========================================================================
test('карта привозит состояние Telegram — отдельной двери для него нет', () => {
  // Тест «карта ходит в ОДНУ дверь» существует не для красоты: patient_card —
  // единственное место, где решается, вправе ли сотрудник видеть пациента.
  // Второй запрос был бы вторым таким местом, и они бы разошлись.
  const { db, pid } = seed();
  const out = patientCard(db, { patient_id: pid }, REGISTRAR);
  assert.ok(Object.prototype.hasOwnProperty.call(out, 'telegram'),
    'состояние Telegram не приехало вместе с картой');
  // Бот в этой базе не настроен — значит именно «выключен», а не «пациент не
  // подключился»: свалить на человека несделанное клиникой нельзя.
  assert.equal(out.telegram.state, 'bot_off');
});

test('сломанный справочник Telegram не мешает открыть карту', () => {
  // Бейдж — справка. Если из-за него перестанет открываться медкарта, лечение
  // встанет из-за украшения.
  const { db, pid } = seed();
  db.exec('DROP TABLE telegram_settings');
  const out = patientCard(db, { patient_id: pid }, REGISTRAR);
