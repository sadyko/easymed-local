...
const READ_ONLY_RPCS = new Set([
  ...
  // …и метка «из заявки» на записях календаря (crm_visit_links) — чтение.
  'crm_visit_links',
  ...
  // файл. Чистое чтение, и для заблокированной клиники это тот самый случай,
  // ради которого исключения здесь вообще есть: история болезни — документ, а
  // не услуга. Пациента, лежащего в койке в день, когда кончилась подписка,
  // всё равно придётся выписывать, а выписывают его по бумагам.
  'admission_case_docs', 'admission_case_file',
  ...
  // STOCK_LOG_V1 / EXPIRY_BALANCE_V1 / MY_STOCK_V1 — журнал движений склада,
  // остатки по партиям и «Мои запасы». Чистое чтение, и та же причина, что у
  // карточки отдела строкой выше: «кто кому что выдал», «что лежит и до какого
  // числа», «что числится за мной» — это уже случившееся прошлое клиники, и
  // закрывать его вместе с записью незачем. Все три идут ОДНОЙ строкой
  // намеренно: они стоят в интерфейсе рядом, и отказ одному при живых соседях
  // читается не как «оплатите подписку», а как «программа сломалась» — вкладка
  // «Журнал» рисует список, соседние «Сроки годности» краснеют ошибкой, а у
  // медсестры «Не удалось загрузить ваши остатки». Приход, выдача и списание —
  // записи, сюда не входят.
  'stock_movements_list', 'stock_expiry_lots', 'holdings_list',
  // STOCK_REQUEST_V1 — минимумы и норма на руках рядом с «Моими запасами».
