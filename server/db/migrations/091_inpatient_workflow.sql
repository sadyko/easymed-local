...
-- ЭТОТ ИНДЕКС СТОИТ ЗДЕСЬ, ДО ПЕРЕСБОРКИ, И ЭТО НЕСУЩЕЕ МЕСТО.
--
-- `invoices.admission_id` завела миграция 040 БЕЗ индекса. `DROP TABLE
-- admissions` ниже выполняется при foreign_keys=ON, а это неявный DELETE всех
-- строк таблицы: на КАЖДУЮ удаляемую госпитализацию SQLite обязана убедиться,
-- что на неё не смотрит ни один счёт, — и без индекса делает это ПОЛНЫМ
-- ПЕРЕБОРОМ `invoices`. Одна госпитализация — один проход по всем счетам
-- клиники. Замерено на этой самой миграции:
--
--     3 000 госпитализаций / 100 000 счетов  →  28.3 с   (с индексом  19 мс)
--     8 000 госпитализаций / 250 000 счетов  →   447 с   (с индексом  39 мс)
--
-- 447 секунд — это семь с половиной минут, в течение которых клиника не
-- поднимается: миграции идут на старте, до первого запроса. Обновление,
-- которое выглядит как «программа зависла», отделение перезагружает силой, а
-- перезапуск посреди миграции — худшее, что может случиться с базой.
--
-- Индекс полезен и ПОСЛЕ пересборки, каждый день: `admissionBalance`
-- (rpc/inpatient.js) спрашивает счета госпитализации двумя запросами, и
-- очередь выписок (`admission_discharge_queue`) зовёт её ПО КАЖДОЙ СТРОКЕ.
-- EXPLAIN QUERY PLAN до: `SCAN invoices` (дважды на вызов); после:
-- `SEARCH invoices USING INDEX idx_invoices_admission (admission_id=?)`.
--
-- IF NOT EXISTS — потому что ТОТ ЖЕ индекс создаёт миграция 098. Он объявлен
-- дважды НАМЕРЕННО: здесь — чтобы каждая БУДУЩАЯ установка проходила 091
-- быстро; в 098 — чтобы его получили машины, на которых 091 УЖЕ ПРИМЕНЕНА и
-- потому переписана быть не может (schema_migrations помнит её имя и второй
-- раз не запустит). Убрать любое из двух объявлений — значит потерять одну из
-- этих половин.
--
-- Пересборки `invoices` здесь нет и быть не может: у неё есть триггеры журнала
-- филиалов (branch-sync/journal.js SHIPPED). CREATE INDEX ни одной строки не
-- трогает и в `sync_journal` не пишет.
...
CREATE TABLE admissions (
  ...
  -- doctor_id остаётся: на него смотрят старые экраны и отчёты. Это ВРАЧ,
  -- НАПРАВИВШИЙ пациента, и перезаписывать его нельзя — иначе исчезнет ответ
  -- на вопрос «кто прислал».
  --
  -- ЗДЕСЬ СТОЯЛО «а RPC держат их согласованными», И ЭТО БЫЛО НЕПРАВДОЙ.
  -- Ни один RPC их не выравнивал: `admission_set_attending` писал только
  -- attending_doctor_id, а консоль койки продолжала ставить в услуги и выдачи
  -- doctor_id — то есть направившего, которому и шли деньги за работу
  -- лечащего. Правило теперь названо честно и держится кодом:
  --
  --   doctor_id            НАПРАВИВШИЙ. Пишется один раз при заявке. RPC его
  --                        НЕ ПЕРЕЗАПИСЫВАЮТ — только ЗАПОЛНЯЮТ, если он пуст
  --                        (COALESCE в admission_set_attending и
  --                        admission_change_attending): у поступившего без
  --                        направления «направивший» отсутствует, и лечащий —
  --                        единственный честный ответ старым отчётам.
  --   attending_doctor_id  ЛЕЧАЩИЙ. Его назначает главный врач и меняет
  --                        admission_change_attending. ЭТО ТА КОЛОНКА, КОТОРУЮ
  --                        ОБЯЗАН ЧИТАТЬ ЛЮБОЙ ЭКРАН, ставящий врача в услугу,
  --                        выдачу или назначение.
