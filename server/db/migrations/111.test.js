// INTERNAL_REFERRAL_V1 (мигр. 111) — наш врач как источник направления.
//
...
test('111: категория внутренних врачей заведена и помечена ФЛАГОМ, а не названием', () => {
  const db = liveDb();
  try {
...
test('111: новый врач сразу становится источником в этой категории', () => {
  const db = liveDb();
  try {
...
test('111: врач без ФИО подписан логином, а не пустой строкой', () => {
  const db = liveDb();
  try {
...
test('111: переименование врача переносится на источник', () => {
...
test('111: уволенный врач перестаёт предлагаться, но источник и его ставки остаются', () => {
  const db = liveDb();
  try {
...
test('111: один источник на врача', () => {
  const db = liveDb();
  try {
...
test('111: врач выбирается тем же двухшаговым списком, что и партнёры', () => {
...
test('111: ставка внутреннего врача читается общим механизмом', () => {
