// REFERRAL_CATEGORY_RATES_V1 (мигр. 109) — перенос уже настроенных денег.
//
...
const SQL = fs.readFileSync(path.join(DIR, '109_referral_category_rates.sql'), 'utf8');

// База БЕЗ 109: применяем всё до неё, чтобы сначала завести «старый мир» —
...
test('109: колонки на месте и по умолчанию источник платит по категории', () => {
  const db = dbBefore109();
  try {
...
test('109: категория из свободного текста становится записью справочника', () => {
  const db = dbBefore109();
  try {
...
test('109: уже существующая категория переиспользуется, а не дублируется', () => {
  const db = dbBefore109();
  try {
...
test('109: два РАЗНЫХ написания дают две категории — миграция не угадывает опечатку', () => {
  const db = dbBefore109();
  try {
...
test('109: правило с именем КАТЕГОРИИ становится её стандартным процентом', () => {
  const db = dbBefore109();
  try {
...
test('109: правило с именем ИСТОЧНИКА переводит его на свою ставку', () => {
  const db = dbBefore109();
  try {
    const id = source(db, 'Иванов Пётр', 'Внешний врач');
...
test('109: правило по имени источника перекрывает правило по имени категории', () => {
...
test('109: ВЫКЛЮЧЕННОЕ правило не переносится', () => {
...
test('109: при нескольких правилах с одним именем берётся последнее заведённое', () => {
...
test('109: правила остаются в базе — это путь назад', () => {
  const db = dbBefore109();
  try {
    source(db, 'Иванов Пётр', 'Внешний врач');
    reward(db, 'Внешний врач', 10);
...
test('109: повторный прогон переноса ничего не меняет', () => {
...
test('109: ставки по группам долетают до базы и объявлены JSON-колонкой', () => {
...
test('109: источник пишет свои ставки и режим тем же путём', () => {
  const db = liveDb();
  try {
