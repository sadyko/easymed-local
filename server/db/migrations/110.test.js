// REFERRAL_SOURCE_CODE_V1 (мигр. 110) — номер источника направления.
//
...
const SQL = fs.readFileSync(path.join(DIR, '110_referral_source_code.sql'), 'utf8');

// База БЕЗ 110 — чтобы завести источники «до номеров» и увидеть, что миграция
// с ними сделала.
...
test('110: номера начинаются с 0001 и идут подряд', () => {
  const db = liveDb();
  try {
...
test('110: номер выдаётся и при заведении через /api/db — тем путём, каким ходит форма', () => {
  const db = liveDb();
  try {
...
test('110: номер выдаётся и регистратору, не только администратору', () => {
...
test('110: удалённый источник НЕ возвращает свой номер следующему', () => {
...
test('110: уже заведённым источникам номера раздаются в порядке заведения', () => {
  const db = dbBefore110();
  try {
...
test('110: заданный вручную номер триггер не перебивает', () => {
...
test('110: после 9999 номер РАСТЁТ, а не ломается', () => {
...
test('110: номер читается через /api/db, но записать его нельзя', () => {
