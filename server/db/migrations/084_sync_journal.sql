...
--
-- ЧЕГО ЭТО НЕ ЗАКРЫВАЕТ. Курсор засева двигается по ВЫГРУЗКЕ, как и раньше:
-- все страницы засева несут один и тот же upto (замороженный пол), и по
-- квитанции их не различить. Пропущенная соседом страница ХОЛОДНОГО засева
-- поэтому по-прежнему не повторяется — это окно первого знакомства, а не
-- ежедневная работа, и закрывать его надо отдельной отметкой на странице,
-- а не втискивать в номер журнала.
