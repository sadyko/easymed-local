...
/**
 * Состояние здания одним словом. Строка на экране ОДНА, поэтому проверки идут
 * ровно в том порядке, в каком они здесь перечислены, — от худшей новости к
 * лучшей, — и занимает строку та беда, которую надо чинить:
 *
 ...
 *   ok       — данные свежие.
 *
 * STALE ВЫШЕ PENDING (ревью Фазы 3). Раньше было наоборот, и это прятало худшую
 * из двух новостей: у здания, молчащего четвёртые сутки, почти всегда висит
 * хоть одна застрявшая запись — и строка показывала спокойное «записи ждут
 * родителя» вместо «данные не приходили с понедельника». Ожидание рассосётся
 * само, как только доедет родитель; молчание здания не рассосётся никогда, и
 * это единственная из двух новостей, по которой человеку надо встать и пойти
 * посмотреть на тот компьютер.
 *
 * @param {object} b строка buildings из report_freshness
 * @param {number} nowMs
 * @param {number} [staleHours]
 * @returns {'own'|'refused'|'never'|'seeding'|'stale'|'pending'|'ok'}
 */
