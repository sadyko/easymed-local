...
export function openQuickPatientModal({
    ...
    const state = { saving: false, finishing: false, api: null };
    ...
    async function finish(patient, { saved = true } = {}) {
        // ОДИН ИСХОД НА ОКНО — И ЭТО НЕ ПЕРЕСТРАХОВКА.
        //
        // Окно ждёт позвавшего ДО КОНЦА (см. ниже), а ждать приходится долго:
        // хвост регистрации заявки, мастер визита, привязка к смете. Всё это
        // время страж дубликатов ещё на экране, и его строки нажимаются: он
        // гасит ТУ САМУЮ строку, по которой щёлкнули, а не выбор целиком.
        // Совпадений бывает два — регистратор щёлкает первую, на экране ничего
        // не меняется, и он щёлкает вторую. Без этой заслонки позвавший получил
        // бы ДВУХ РАЗНЫХ пациентов на одно заведение (заявка колл-центра
        // привязалась бы к чужой карте), а окно снялось бы дважды.
        //
        // Признак снимается только вместе с окном: второго исхода у него нет.
        ...
        try {
            ...
            if (!force && e && e.code === 'DUPLICATE_PATIENT' && e.existing) {
                openDuplicatePatientDialog(e, {
                    ...
                    // «Создать принудительно» — ПО ТОМУ ЖЕ ПРАВИЛУ, ЧТО И
                    // «Открыть существующего»: не вышло — выбор остаётся.
                    // runSave на отказе записи отдаёт null, и отданный наружу
                    // как есть он читался диалогом как «готово» (закрыто всё,
                    // кроме undefined/false). Человек видел отказ, а под ним
                    // пустой экран: ни выбора, ни набранной карты.
                    onForceCreate: async () => {
                        const created = await runSave(payload, true);
                        return !!created;
                    },
                });
                return null;
