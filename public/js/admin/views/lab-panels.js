...
//
// ── inherited from lab-settings.js, unchanged ──────────────────────────────
...
export async function mountLabPanels(container) {
    ...
    // A11Y — the box carries no visible label, so without aria-label a screen
    // reader announces it as an unnamed text field. spellcheck off: panel names
    ...
    // the track refuses to shrink below the table's intrinsic width and the whole
    ...
    function paintList() {
        clear(listEl);
        // Loading, empty and «nothing found» are three different answers; before
        // this the first two looked identical and the screen quietly claimed the
        // clinic had no panels while the read was still in flight.
        if (state.loading) { listEl.appendChild(h('div', { class: 'muted', style: { padding: '24px 12px', fontSize: '12.5px' } }, 'Загрузка…')); return; }
    ...
    function paintEditor() {
        clear(editorEl);
        if (state.loading) { editorEl.appendChild(h('div', { class: 'empty', style: { padding: '54px 20px', textAlign: 'center' } }, 'Загрузка…')); return; }
        const p = state.selected;
            ...
            rows.forEach((rr, i) => {
                ...
                listEl.appendChild(h('tr', null,
                    ...
                    h('td', null, h('span', { class: 'lp-actions' },
                        ...
                        h('button', { class: 'lp-ic', type: 'button', title: 'Вниз', 'aria-label': 'Вниз', onclick: () => { if (i < rows.length - 1) { const t = rows[i + 1]; rows[i + 1] = rows[i]; rows[i] = t; paintRows(); } } }, '↓'),
                        h('button', { class: 'lp-ic del', type: 'button', title: 'Удалить', 'aria-label': 'Удалить', onclick: () => { rows.splice(i, 1); paintRows(); } }, Icon('Trash', { size: 12 }))))));
            });
        };
        paintRows();
    ...
    // empty editor is indistinguishable from a clinic that has no panels yet.
    paintList();
    paintEditor();
    await reload();
}
