...
function openRejectDialog(patient, onConfirm) {
    ...
    const errEl = h('div', { style: { fontSize: '12.5px', color: 'var(--crit-700, #b91c1c)', minHeight: '14px', marginTop: '2px' } });
    const ta = h('textarea', { rows: '3', placeholder: 'Например: пациент не отвечает, дубль записи, нет нужного врача в эту дату…',
        style: { width: '100%', boxSizing: 'border-box', border: '1px solid var(--ink-200, #d3d9de)', borderRadius: '10px', padding: '10px 12px', font: 'inherit', fontSize: '13.5px', resize: 'vertical', minHeight: '74px' },
    ...
    overlay.appendChild(h('div', { class: 'modal-card', style: { width: '440px', maxWidth: 'calc(100vw - 32px)' } },
        ...
        h('div', { class: 'modal-body' },
            ...
            h('div', { style: { display: 'grid', gap: '5px' } },
                h('label', { style: { fontSize: '12.5px', fontWeight: 600, color: 'var(--ink-500)' } }, 'Причина отказа'),
                ta, errEl)),
...
export async function renderRequestsInbox(container) {
    ...
    function hero() {
        return h('div', { style: {
            ...
            h('div', { style: { position: 'relative', display: 'inline-flex', alignItems: 'center', gap: '7px', fontSize: '12.5px', fontWeight: 600, background: 'rgba(255,255,255,0.14)', padding: '4px 10px', borderRadius: '999px', marginBottom: '10px' } },
                Icon('Bell', { size: 13 }), 'Symptex'),
            h('h2', { style: { margin: 0, fontSize: '24px', fontWeight: 700, letterSpacing: '-0.02em', position: 'relative' } }, 'Заявки'),
    ...
    function statCard({ label, value, sub, color, icon, mode }) {
        return h('div', { class: 'stat', style: { minWidth: 0, cursor: 'pointer', outline: state.mode === mode ? `2px solid ${color}` : 'none' },
            ...
            sub ? h('div', { style: { fontSize: '12.5px', color: 'var(--ink-500)', marginTop: '4px' } }, sub) : null);
    }
    function kpiRow(c) {
    ...
    function emptyState() {
        ...
        return h('div', { class: 'card', style: { padding: '48px 24px', textAlign: 'center', color: 'var(--ink-400)' } },
            ...
            h('div', { style: { fontWeight: 700, fontSize: '17px', color: 'var(--ink-800, #142026)' } }, active ? 'Новых заявок нет' : 'История пуста'),
    ...
    function patientHead(v, lk, rightBadges) {
        ...
        return h('div', { style: { display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' } },
            h('span', { style: { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '36px', height: '36px', borderRadius: '10px', background: 'var(--primary-50, #effaf8)', fontWeight: 700, fontSize: '12.5px', color: 'var(--primary-700, #115d5a)', flex: 'none' } }, initials(patient)),
            h('div', { style: { minWidth: 0 } },
                h('div', { style: { fontWeight: 700, fontSize: '15px', color: 'var(--ink-900, #0b1418)' } }, patient),
    ...
    const pill = (label, fg, bg) => h('span', { style: { fontSize: '12.5px', fontWeight: 600, color: fg, background: bg, padding: '3px 10px', borderRadius: '999px', whiteSpace: 'nowrap' } }, label);
    ...
    function metaLine(v, lk) {
        ...
        return h('div', { style: { fontSize: '12.5px', color: 'var(--ink-500, #55636d)', display: 'flex', gap: '14px', flexWrap: 'wrap' } },
    ...
    function activeCard(v, lk) {
        ...
        const patient = lk.pm[v.patient_id] || 'Пациент';
        const hasSlot = v.visit_date && !/^0001/.test(String(v.visit_date));
        const inp = { fontSize: '13.5px', padding: '8px 10px', border: '1px solid var(--ink-200, #d3d9de)', borderRadius: '9px', background: '#fff', color: 'var(--ink-800, #142026)' };
        ...
        return cardShell(h('div', { style: { display: 'flex', gap: '16px', flexWrap: 'wrap', justifyContent: 'space-between' } },
            h('div', { style: { minWidth: '260px', flex: '1 1 320px' } },
                ...
                h('div', { style: { fontSize: '13.5px', fontWeight: 600, color: 'var(--ink-800, #142026)', margin: '2px 0 6px' } }, serviceText(v, lk)),
                metaLine(v, lk)),
            h('div', { style: { display: 'flex', flexDirection: 'column', gap: '7px', minWidth: '210px' } },
                h('label', { style: { fontSize: '12.5px', color: 'var(--ink-400)', fontWeight: 600 } }, 'Врач'),
                docSel,
                h('label', { style: { fontSize: '12.5px', color: 'var(--ink-400)', fontWeight: 600, marginTop: '2px' } }, 'Кабинет (для услуг без врача)'),
                roomSel,
                h('label', { style: { fontSize: '12.5px', color: 'var(--ink-400)', fontWeight: 600, marginTop: '2px' } }, 'Дата и время приёма'),
                dtInput,
    ...
    function historyCard(v, lk) {
        ...
        return cardShell(h('div', null,
            ...
            (v.status === 'cancelled' && (v.cancel_reason || who)) ? h('div', { style: { marginTop: '9px', padding: '9px 12px', background: 'var(--crit-50, #fef2f2)', border: '1px solid var(--crit-200, #fecaca)', borderRadius: '10px', fontSize: '12.5px', color: 'var(--crit-700, #b91c1c)' } },
                ...
                (who || v.cancelled_at) ? h('div', { style: { color: 'var(--ink-400, #7a8892)', marginTop: '3px', fontSize: '12.5px' } }, [who ? trf('отклонил: {who}', { who }) : '', v.cancelled_at ? ('· ' + fmtRel(v.cancelled_at)) : ''].filter(Boolean).join(' ')) : null) : null));
    }
