    ...
    return h('div', { style: { display: 'flex', flexDirection: 'column', gap: '16px' } },
        ...
        h('div', { class: 'card' },
            h('div', { class: 'card-header' },
                ...
                h('span', { class: 'muted', style: { fontSize: '12.5px' } }, 'order → distributed'),
            ),
                    ...
                    ...PACS_STUDIES
                        ...
                        .map(s => h('div', { class: 'row', style: { padding: '10px 18px', borderTop: '1px solid var(--ink-100)', gap: '12px', cursor: 'pointer' }, onclick: () => openInViewer(s) },
                            ...
                            h('div', { style: { flex: 1, minWidth: 0 } },
                                ...
                                h('div', { class: 'muted', style: { fontSize: '12.5px' } }, `${s.bodyPart} · ${s.images} img`),
                            ),
        ...
        h('div', { class: 'card-header' },
            h('div', { class: 'row', style: { gap: '6px', flexWrap: 'wrap' } },
                ...buckets.map(b => h('button', {
                    ...
                    b.label,
                    h('span', { style: {
                        fontSize: '12.5px', padding: '0 6px', borderRadius: '999px',
            ...
            h('tbody', null, ...shown.map(s => h('tr', { style: { cursor: 'pointer' }, onclick: () => openInViewer(s) },
                ...
                h('td', null, h('div', { class: 'row', style: { gap: '10px' } },
                    ...
                    h('div', null,
                        ...
                        h('div', { class: 'muted cell-mono', style: { fontSize: '12.5px' } }, `${s.patient.mrn} · ${s.patient.age}${s.patient.sex}`),
                    ),
                )),
                h('td', { class: 'cell-strong', style: { maxWidth: '220px' } }, s.bodyPart),
                h('td', { class: 'cell-mono muted', style: { fontSize: '12.5px' } }, s.acc),
                h('td', { class: 'muted' }, s.ref),
                h('td', { class: 'num' }, String(s.images)),
                h('td', null, s.priority === 'stat'
                    ? h('span', { class: 'pacs-stat' }, 'STAT')
                    : h('span', { class: 'muted', style: { fontSize: '12.5px' } }, 'Routine')),
                h('td', null, pacsStatus(s.status)),
                h('td', { class: 'muted num', style: { fontSize: '12.5px' } }, s.when),
        ...
        h('div', { class: 'card', style: { alignSelf: 'flex-start' } },
            ...
            h('div', { style: { padding: '16px' } },
                h('div', { class: 'row', style: { gap: '12px', marginBottom: '14px' } },
                    modBadge(study.mod),
                    h('div', null,
                        h('div', { class: 'cell-strong', style: { fontSize: '13.5px' } }, study.patient.name),
                        h('div', { class: 'muted', style: { fontSize: '12.5px' } }, `${study.patient.mrn} · ${study.patient.age}${study.patient.sex} · ${study.acc}`),
                    ),
                ),
                h('div', { style: { fontSize: '13.5px', fontWeight: 600, color: 'var(--ink-900)', marginBottom: '10px' } }, study.bodyPart),
        ...
        h('div', { class: 'card' },
            ...
            h('div', { style: { padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: '16px' } },
                ...
                h('label', { class: 'pr-critical' },
                    ...
                    h('div', null,
                        h('div', { style: { fontSize: '13.5px', fontWeight: 700, color: state.report.critical ? 'var(--crit-700)' : 'var(--ink-900)' } }, 'Critical / urgent finding'),
                        h('div', { class: 'muted', style: { fontSize: '12.5px' } }, 'Triggers immediate call-back to the referring doctor and a flag on the patient record.'),
                    ),
                ),
                h('div', { class: 'pr-foot' },
                ...
                h('div', { class: 'row', style: { gap: '12px' } },
                    ...
                    h('div', { style: { flex: 1, minWidth: 0 } },
                        h('div', { class: 'row', style: { gap: '8px' } },
                            h('span', { style: { fontSize: '15px', fontWeight: 700, color: 'var(--ink-900)' } }, e.name),
                            h('span', { style: { display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '2px 9px', borderRadius: '999px', background: s.bg, color: s.color, fontSize: '12.5px', fontWeight: 700 } },
                                h('span', { style: { width: '6px', height: '6px', borderRadius: '999px', background: s.color } }), s.label),
                        ),
                        h('div', { class: 'muted', style: { fontSize: '12.5px', marginTop: '2px' } }, e.room),
                    ),
                ),
...
function pacsStatus(status) {
    ...
    return h('span', {
        style: {
            ...
            fontSize: '12.5px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', whiteSpace: 'nowrap',
        },
    },
    ...
    return h('div', { style: { position: 'relative', width: size + 'px', height: size + 'px' } },
        ...
        h('div', { style: { position: 'absolute', inset: 0, display: 'grid', placeItems: 'center' } },
            h('div', { style: { textAlign: 'center' } },
                ...
                h('div', { class: 'muted', style: { fontSize: '12.5px', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 600 } }, sub),
            ),
        ),
    );
}
