...
// -----------------------------------------------------------------------------
// Дата рождения — LAB_CARD_V3.
//
// На снимке владельца в узбекском интерфейсе стояло «1994 M11 15». Это не
// «сырая дата»: это то, что Intl отдаёт для КОРНЕВОЙ локали, когда в сборке
// браузера нет данных запрошенного языка. Общий формат приложения (fmtDate,
// DATE_FMT_V1 в ui.js) остаётся источником — он же даёт «15 ноября 1994» там,
// где данные есть; но карточка обязана быть читаемой и на машине без них.
//
// Поэтому: сначала общий формат, и только если он выдал машинную форму —
// собираем дату из названий месяцев, которые и так лежат в словаре
// (i18n-strings.js: «ноября» → uz «noyabr», en «November»).
// -----------------------------------------------------------------------------
export const RU_MONTH_GEN = ['января', 'февраля', 'марта', 'апреля', 'мая', 'июня',
    'июля', 'августа', 'сентября', 'октября', 'ноября', 'декабря'];

/** 'YYYY-MM-DD…' -> { y, m (0-11), d } либо null. */
...
/** Корневая форма Intl — «1994 M11 15». Человеку она ничего не говорит. */
export function isMachineDate(s) {
    return /^\d{4}\s+M\d{1,2}\s+\d{1,2}$/.test(String(s == null ? '' : s).trim());
}
