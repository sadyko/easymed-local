...
function waitingBedCard(list, reload, onNavigate) {
    ...
    return listCard(tr('Ждут размещения'), 'Clock', list.length,
        ...
        // ПУСТО — ЭТО НЕ СЛОМАНО, и сказать это надо здесь же. Отказ запроса
        // рисует другую карточку (см. paint выше: «Список госпитализаций не
        // загрузился», причина, кнопка «Повторить») — но человек, впервые
        // открывший раздел, ни разу не видел той карточки и не знает, что
        // разница есть. Поэтому пустая очередь называет ОБА входа, которыми
        // заявка сюда попадает: пока их не назвали, «никого не ждут» и «не
        // доехало» читались с экрана одинаково.
