    ...
    return h('div', { class: 'card', style: { padding: '12px 14px' } },
        h('div', { style: { display: 'flex', gap: '16px', flexWrap: 'wrap', alignItems: 'center' } },
            h('div', { style: { display: 'flex', alignItems: 'center', gap: '8px' } },
                ...
                h('span', { style: { fontSize: '12.5px', fontWeight: 700, color: 'var(--ink-700)', textTransform: 'uppercase', letterSpacing: '0.06em' } }, 'View'),
            ),
            ...
            h('span', { style: { fontSize: '12.5px', fontWeight: 700, color: 'var(--ink-700)', textTransform: 'uppercase', letterSpacing: '0.06em' } }, 'Period'),
...
function hospitalWeather(m) {
    ...
    const metric = (lbl, value, opts = {}) => h('div', { style: { display: 'flex', flexDirection: 'column', gap: '2px', minWidth: '110px' } },
        h('div', { style: { fontSize: '12.5px', textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--ink-500)', fontWeight: 700 } }, lbl),
    ...
    return h('div', { class: 'card', style: { padding: '20px 24px', display: 'flex', alignItems: 'center', gap: '32px', flexWrap: 'wrap' } },
        h('div', { style: { display: 'flex', alignItems: 'baseline', gap: '14px', minWidth: '260px' } },
            h('div', null,
                h('div', { class: 'row', style: { gap: '8px', alignItems: 'center', color: 'var(--ink-500)', fontSize: '12.5px', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 700 } },
                ...
                h('div', { style: { display: 'flex', alignItems: 'baseline', gap: '8px', marginTop: '6px' } },
                    h('span', { class: 'num', style: { fontSize: '40px', fontWeight: 700, color: c.rgb, lineHeight: '1', letterSpacing: '-0.02em' } }, String(score)),
                ...
                h('div', { style: { marginTop: '6px' } },
                    h('span', { style: { display: 'inline-block', padding: '2px 10px', borderRadius: '999px', background: c.rgbA(0.12), color: c.rgb, fontSize: '12.5px', fontWeight: 600 } }, label),
                ),
            ),
        ),
...
function pulseStrip(m) {
    ...
    return h('div', { class: 'card', style: { padding: '14px 16px' } },
        h('div', { style: { fontSize: '12.5px', textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--ink-600)', fontWeight: 700, marginBottom: '10px' } }, "Right now"),
        h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: '14px' } },
            ...cells.map(c => h('div', null,
                h('div', { class: 'row', style: { gap: '6px', color: c.color, fontSize: '12.5px', fontWeight: 600 } }, Icon(c.icon, { size: 13 }), c.label),
                h('div', { class: 'num', style: { fontSize: '24px', fontWeight: 700, color: 'var(--ink-900)', marginTop: '4px', letterSpacing: '-0.01em' } }, c.value),
            )),
        ),
    );
}
...
function heroKpis(m) {
    ...
    return h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '14px' } },
        ...cards.map(c => h('div', { class: 'card', style: { padding: '16px 18px' } },
            h('div', { style: { fontSize: '12.5px', textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--ink-600)', fontWeight: 700 } }, c.label),
            h('div', { class: 'num', style: { fontSize: '24px', fontWeight: 700, color: 'var(--ink-900)', marginTop: '6px', letterSpacing: '-0.02em' } }, c.value),
            h('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '6px' } },
                h('div', { class: 'muted', style: { fontSize: '12.5px' } }, c.sub),
            ...
            (c.spark && c.spark.length > 1)
                ...
                : h('div', { class: 'muted', style: { fontSize: '12.5px', marginTop: '8px', height: '38px', display: 'flex', alignItems: 'center' } }, ''),
        )),
    );
}
    ...
    if (alerts.length === 0) {
        return h('div', { class: 'card', style: { padding: '16px 18px' } },
            h('div', { class: 'row', style: { gap: '10px', alignItems: 'center' } },
                ...
                h('div', null,
                    h('div', { style: { fontWeight: 700, fontSize: '13.5px', color: 'var(--ink-900)' } }, 'All clear'),
                    h('div', { class: 'muted', style: { fontSize: '12.5px', marginTop: '2px' } }, 'No outstanding alerts right now.'),
                ),
            ),
        );
    }
    ...
    return h('div', { class: 'card' },
        ...
        h('div', { style: { display: 'flex', flexDirection: 'column' } },
            ...alerts.map(a => h('div', { style: { display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', borderTop: '1px solid var(--ink-100)', background: a.sev === 'crit' ? 'var(--crit-50)' : a.sev === 'warn' ? 'var(--warn-50)' : 'white' } },
                ...
                h('div', { style: { flex: 1 } },
                    h('div', { style: { fontWeight: 600, fontSize: '13.5px', color: 'var(--ink-900)' } }, a.label),
                    h('div', { class: 'muted', style: { fontSize: '12.5px', marginTop: '2px' } }, a.hint),
                ),
        ...
        h('div', { style: { display: 'grid', gridTemplateColumns: '1.25fr 1fr', gap: '16px' } },
            ...
            h('div', { class: 'card' },
                h('div', { class: 'card-header' },
                    ...
                    h('span', { class: 'muted', style: { fontSize: '12.5px' } }, collectionPct + '% collected'),
                ),
                h('div', { style: { padding: '18px 22px 22px' } },
                    ...
                    h('div', { style: { marginTop: '18px', padding: '12px 14px', background: gap > 0 ? 'var(--warn-50)' : 'var(--ok-50)', border: '1px solid ' + (gap > 0 ? '#fde9b6' : '#c6efd9'), borderRadius: '9px' } },
                        h('div', { class: 'row', style: { gap: '8px', color: gap > 0 ? 'var(--warn-700)' : 'var(--ok-700)' } },
                            ...
                            h('span', { class: 'grow' }),
                            h('span', { style: { fontSize: '12.5px', color: 'var(--ink-600)' } },
            ...
            h('div', { class: 'card' },
                h('div', { class: 'card-header' },
                    ...
                    h('span', { class: 'muted', style: { fontSize: '12.5px' } }, formatMoney(rev)),
                ),
                        ...
                        : h('div', null,
                            ...
                            h('div', { style: { display: 'flex', flexDirection: 'column', gap: '10px' } },
                                ...areas.map(a => h('div', { class: 'row', style: { fontSize: '12.5px', gap: '8px' } },
                                    ...
                                    h('span', { class: 'muted num', style: { fontSize: '12.5px', minWidth: '34px', textAlign: 'right' } }, Math.round(a.val / areaTotal * 100) + '%'),
                                )),
                            ),
                        ),
                ),
...
function moneyBar(label, value, max, color, sub) {
    return h('div', null,
        h('div', { class: 'row', style: { marginBottom: '6px', fontSize: '13.5px' } },
        ...
        h('div', { class: 'muted', style: { fontSize: '12.5px', marginTop: '4px' } }, sub),
    );
}
        ...
        h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '16px' } },
            ...
            h('div', { class: 'card' },
                h('div', { class: 'card-header' },
                    ...
                    h('span', { class: 'muted', style: { fontSize: '12.5px' } }, (spark.reduce((s, v) => s + v, 0)) + ' total'),
                ),
            ...
            h('div', { class: 'card' },
                h('div', { class: 'card-header' },
                    ...
                    h('span', { class: 'muted', style: { fontSize: '12.5px' } }, 'Completed work · ' + periodSubtitle()),
                ),
                rows.length === 0
                    ...
                    : h('div', { style: { padding: '8px 0 4px' } },
                        ...rows.map((r, i) => h('div', { style: { padding: '10px 22px', borderTop: i > 0 ? '1px solid var(--ink-100)' : 'none' } },
                            h('div', { class: 'row', style: { marginBottom: '6px', fontSize: '13.5px' } },
                                ...
                                h('span', { class: 'muted num', style: { fontSize: '12.5px', marginLeft: '8px' } }, (r.count || 0) + '×'),
                            ),
                ...
                h('tbody', null,
                    ...
                    h('tr', { style: { background: 'var(--ink-25)' } },
                        h('td', { class: 'cell-strong' }, h('div', { class: 'row', style: { gap: '10px' } },
                            h('div', { style: { width: '34px', height: '34px', borderRadius: '999px', background: 'linear-gradient(135deg, var(--primary-500), var(--primary-700))', color: 'white', display: 'grid', placeItems: 'center', fontWeight: 700, fontSize: '12.5px' } }, '∑'),
        ...
        h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' } },
            ...
            h('div', { class: 'card' },
                h('div', { class: 'card-header' },
                    ...
                    h('span', { class: 'muted', style: { fontSize: '12.5px' } }, `${m.beds_occupied || 0} of ${m.beds_total || 0} beds`),
                ),
            ...
            h('div', { class: 'card' },
                h('div', { class: 'card-header' },
                    ...
                    h('span', { class: 'muted', style: { fontSize: '12.5px' } }, periodSubtitle()),
                ),
                h('div', { style: { padding: '14px 22px 20px', display: 'flex', flexDirection: 'column', gap: '12px' } },
                    docs.length === 0
                        ...
                        : docs.map(d => h('div', null,
                            h('div', { class: 'row', style: { marginBottom: '5px', fontSize: '12.5px' } },
                                ...
                                h('span', { class: 'muted', style: { fontSize: '12.5px', marginLeft: '6px' } }, `· ${d.count}×`),
                                h('span', { class: 'grow' }),
        ...
        h('div', { class: 'card', style: { padding: '18px 22px' } },
            h('div', { class: 'row', style: { gap: '12px', alignItems: 'center' } },
                ...
                h('div', { style: { flex: 1 } },
                    ...
                    h('div', { class: 'muted', style: { fontSize: '12.5px', marginTop: '2px' } }, 'Patient satisfaction, wait time and safety incidents will appear here once feedback collection is connected.'),
                ),
                Tag('No data', { kind: '' }),
            ),
        ),
...
function decisionSummary(m) {
    ...
    const col = (title, icon, accent, items, bullet, bulletColor) => h('div', { style: { background: 'white', border: '1px solid var(--ink-100)', borderRadius: '14px', padding: '20px 22px', borderLeft: '4px solid ' + accent } },
        ...
        h('ul', { style: { margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '10px' } },
            ...items.map(t => h('li', { style: { display: 'flex', gap: '10px', fontSize: '13.5px', color: 'var(--ink-700)', lineHeight: '1.5' } },
...
function sectionHeader(icon, title, sub) {
    return h('div', { class: 'row', style: { gap: '12px', marginBottom: '12px', marginTop: '8px' } },
        ...
        h('h2', { style: { margin: 0, fontSize: '17px', fontWeight: 700, color: 'var(--ink-900)', letterSpacing: '-0.01em' } }, title),
        h('span', { class: 'muted', style: { fontSize: '12.5px', marginLeft: '4px' } }, '· ' + sub),
    );
}
