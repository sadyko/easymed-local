...
//
// Правило перехода — дословно серверное (grants.js): пока роль ключ НЕ
// настраивала, grantLevel() отвечает null и решает прежний ключ. Ни одна
// клиника после обновления не теряет кнопку, которую имела.
export function canCreatePatient() {
    if (grantLevel('crm.convert') !== null) return grantAllows('crm.convert', 'edit');
