...
function assertPanelEditor(root, where) {
  ...
  // LIS_INGEST_V1 — сопоставление с прибором обязано быть НА ЭКРАНЕ, а не
  // только в коде. Выбор анализатора один раз уже существовал переменной,
  // читался при сохранении и нигде не отрисовывался: колонка «Поле
  // анализатора» у каждой строки писала «выберите анализатор», а выбрать его
  // было негде — настроить приём результатов стало невозможно.
  // Проверяем ПОДПИСЬ выбора, а не слово «Анализатор»: оно есть и на кнопке
  // переключателя режимов, поэтому такая проверка прошла бы и со сломанным
  // экраном. Подпись рисуется только рядом с самим выпадающим списком.
  assert.ok(text.includes('Выберите прибор') || text.includes('Приборов пока нет'),
    where + ': выбор анализатора панели отсутствует на экране');
  assert.ok(text.includes('Поле анализатора'), where + ': колонка сопоставления у показателей');
}
...
test('режим «Статистика»: чипы периода (по умолчанию 30 дней), два блока счётчиков, RPC зовётся с периодом', async () => {
  ...
  // Ровно один ЛАБОРАТОРНЫЙ вызов RPC, период проговорён явно.
  //
  // Фильтр по префиксу, а не равенство всему списку: в шапке экрана теперь
  // живёт кнопка «Синхронизация» (BRANCH_SYNC_HOURLY_V1), которая один раз за
  // сессию спрашивает роль установки. Проверять здесь ВЕСЬ список значило бы
  // ронять тест про статистику всякий раз, когда в шапку что-то добавили, —
  // и, что хуже, ставить его результат в зависимость от того, успел ли другой
  // тест в этом файле прогреть кэш роли раньше.
  assert.deepStrictEqual(rpcCalls.filter((c) => c.name.startsWith('lab_')),
    [{ name: 'lab_usage_stats', args: { period: '30d' } }]);
