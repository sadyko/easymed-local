...
test('колонка меню больше не отдельная поверхность — ни подложки, ни границы, ни тени', () => {
    ...
    // WHITE_GROUND_GLOW_V1 — правило СМЕНИЛОСЬ, а не отменилось.
    // Было: грунт обязан отличаться от белого, потому что именно он показывал
    // рабочее окно. Стало (владелец: «make background white, but the frames
    // should have little glow of green»): грунт белый, и окно показывает
    // ТОЛЬКО его рамка. Значит проверять надо её — если рамка побледнеет,
    // окно исчезнет так же, как исчезало от светлого грунта.
    const frame = rule(MAIN, '.card').border.match(/var\(--[a-z0-9-]+\)|#[0-9a-fA-F]{3,8}/);
    assert.ok(frame, 'у рабочего окна пропала рамка — на белом грунте её больше нечем заменить');
    assert.ok(ratio(frame[0], ground) >= 1.35,
        `рамка окна на грунте ${ratio(frame[0], ground)}:1 — окна снова не видно`);
...
test('поле страницы НЕ белое, а рабочее окно — белое: их видно друг на друге', () => {
    ...
    // WHITE_GROUND_GLOW_V1 — грунт теперь БЕЛЫЙ намеренно. Прежняя проверка
    // («поле не белое») сторожила механизм, которого больше нет: окно
    // отделялось цветом поля. Теперь его отделяет рамка, и сторожить надо её.
    // Само равенство «поле = окно = белое» ошибкой быть перестало.
    assert.strictEqual(hex(ground), '#ffffff',
        'грунт страницы больше не белый — так задумано было в прежнем оформлении, а не в этом');
    ...
    // WHITE_GROUND_GLOW_V1 — из трёх опор окна (светлота, рамка, тень) первая
    // убрана намеренно: поле и окно теперь одного цвета. Проверять «окно
    // светлее поля» больше нельзя — это всегда 1:1. Остаются две, и обе
    // проверены здесь же ниже: рамка видна с обеих сторон, тень задана
    // токеном и имеет кольцо. Само равенство фиксируем, чтобы смена грунта
    // где-то ещё не прошла молча.
    assert.strictEqual(ratio(card.background, ground), 1,
        'поле и окно снова разного цвета — тогда вернулась и старая опора, и её надо проверять как раньше');

    // Верхняя панель — хром, а не поверхность: белой полосой она достраивала
    // тот же сплошной лист сверху.
    const bar = rule(MAIN, '.appbar');
    // WHITE_GROUND_GLOW_V1 — «панель не белая» сторожило не цвет, а ОТДЕЛЬНУЮ
    // ПОВЕРХНОСТЬ: белая полоса на сером поле достраивала сплошной лист сверху.
    // Теперь поле само белое, и запрет на белый потерял смысл; смысл остался в
    // том, что панель не имеет СВОЕЙ подложки и лежит на грунте.
    assert.strictEqual(hex(bar.background), hex(ground),
        'верхняя панель получила собственную подложку — она хром, а не поверхность');
...
test('у рабочего окна есть видимая рамка и тень — и они на токенах, а не на глаз', () => {
    ...
    // WHITE_GROUND_GLOW_V1 — форма тени сменилась: раньше это была ОДНА мягкая
    // тень (размытие 24px), она и приподнимала белое окно над серым полем.
    // На белом поле приподнимать не над чем, и работу делает контур: плотное
    // кольцо без размытия плюс короткий мягкий ореол. Поэтому требование
    // «размытие ≥12px» заменено на «кольцо есть И ореол мягкий»: без кольца
    // граница пропадёт, без ореола станет резкой чертой.
    const ring = /0\s+0\s+0\s+(\d+)px/.exec(shadow[1]);
    assert.ok(ring && Number(ring[1]) >= 2,
        `у окна нет плотного кольца (${shadow[1]}) — на белом поле границу держать нечем`);
    assert.ok(Math.max(...blurs) >= 8,
        `ореол окна размыт на ${Math.max(...blurs)}px — граница читается резкой чертой`);
});
