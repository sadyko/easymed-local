...
// DOC_PAGINATE_V1 меняет .sheet с flex-колонки на обычный блок в печати, чтобы
// страницы делились правильно. Побочное следствие: margin-top:auto у подвала
// вне flex обращается в НОЛЬ, и подвал прилипает к последнему блоку. На экране
// этого не видно — там колонка, и auto отжимает подвал вниз, — поэтому ошибка
// живёт только на бумаге.
test('в печати подвал отделён от содержимого', () => {
  const html = act({ queue: [{ service: 'CA 15-3 (Онкомаркер)', label: 'Лаборатория', number: 1 }] });
  const at = html.indexOf('@media print');
  assert.notEqual(at, -1, 'печатные правила на месте');
  assert.match(html.slice(at), /\.footer\s*\{[^}]*margin-top:\s*\d/,
    'у подвала должен быть НАСТОЯЩИЙ отступ сверху в печати, а не auto');
});
