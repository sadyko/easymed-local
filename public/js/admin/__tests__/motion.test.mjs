...
// SHELL_REVEAL_V1 — появление просит ОБОЛОЧКА после каждого экрана, поэтому
// новый раздел получает его, не зная о помощнике вовсе.
test('оболочка просит появление для каждого экрана — после отрисовки, из одного места', () => {
    const src = read('js/admin.js');
    assert.ok(/revealOn\(/.test(src), 'оболочка не зовёт общего помощника');
    assert.ok(/function revealShell\(root\)/.test(src), 'нет revealShell');
    assert.ok(/finally \{[^}]*revealShell\(root\)/.test(src), 'появление не привязано к концу отрисовки экрана (finally)');
    assert.ok(!/new IntersectionObserver/.test(src), 'оболочка завела СВОЙ наблюдатель');
    // Класс помощника в оболочке не пишется буквой: селектор собирается из
    // его же констант, иначе страж «скрывающий класс не в разметке» ослеп бы.
    assert.ok(/HIDDEN_CLASS/.test(src) && /SHOWN_CLASS/.test(src), 'селектор оболочки написан буквами, а не константами помощника');
});
