...
test('состояние здания: худшая новость занимает единственную строку', () => {
  ...
  // Ровно тот случай, ради которого порядок и переставлен (ревью Фазы 3): у
  // молчащего здания почти всегда висит застрявшая запись, и «записи ждут
  // родителя» занимало строку вместо «данные не приходили четвёртые сутки».
  assert.equal(freshnessState(fresh({ last_received: hoursAgo(96), pending: 1 }), NOW), 'stale',
    'ожидание рассосётся само, молчание здания — никогда');
});
...
// Записи, чью букву прочитать не удалось, сервер считает ОТДЕЛЬНО — именно
// потому, что приписать их зданию наугад нельзя. Экран их не рисовал вовсе:
// единственная строка, говорящая «мы не смогли понять, чьё это», молчала, а
// вместе с ней молчали и её записи — их нет ни в одной строке зданий.
test('записи без здания названы, а не растворены в строках зданий', () => {
  assert.match(hub, /data\.pending_unattributed \|\| data\.refused_unattributed/,
    'строка появляется, только когда такие записи есть');
  assert.match(hub, /tr\('Здание не определено'\)/);
  assert.match(hub, /pending: data\.pending_unattributed \|\| 0/);
  assert.match(hub, /refused: data\.refused_unattributed \|\| 0/);
});
...
test('фразы свежести переводятся целиком, вместе со своими дырками', () => {
  const keys = [
    ...
    'Здание не определено',
    'Записи есть, но чьи они — прочитать не удалось · ждут: {pending} · не приняты: {refused}',
    'ещё едет: {amount}',
