...
test('между двумя соседними карточками лежит полоса — другого цвета и измеримой высоты', async () => {
  ...
  // WHITE_GROUND_GLOW_V1 — МЕХАНИЗМ СМЕНИЛСЯ, требование осталось.
  // Раньше соседние карточки разделяла ЗАЛИТАЯ полоса: под ними лежал серый
  // грунт. Владелец попросил белый фон, грунт стал белым, и полосы больше нет
  // — 1.00:1, ровно то состояние, которое когда-то и признали дефектом.
  // Теперь карточки разделяет их СОБСТВЕННАЯ рамка, и проверять надо её:
  // если рамка побледнеет, две соседние пробы снова сольются в одну.
  const strip = contrast(GAP_BG, CARD_BG);
  const frame = contrast(CARD_EDGE, CARD_BG);
  assert.ok(strip >= 1.15 || frame >= 1.35,
    'карточки нечем разделить: полоса ' + round2(strip) + ':1, рамка ' + round2(frame) + ':1 — '
    + 'на белом грунте всю работу делает рамка, и она пропала');
});
...
test('приглушённая неоплаченная карточка — всё ещё карточка, а её пунктир не спутать с разделителем', async () => {
  ...
  // WHITE_GROUND_GLOW_V1 — заливка неоплаченной (ink-25) на белом грунте почти
  // неотличима от него, и это НОРМАЛЬНО: приглушённой она и задумана. Её
  // держит пунктирный край — он проверяется следующим пунктом и на белом даёт
  // 2.11:1, вдвое громче рамки обычной карточки. Проверяем именно то, что
  // теперь несёт смысл: приглушённая заливка не ТЕМНЕЕ обычной карточки,
  // иначе «работы здесь нет» начнёт читаться как «здесь важное».
  const toGap = contrast(UNPAID_BG, GAP_BG);
  assert.ok(toGap <= 1.15,
    'неоплаченная карточка (' + UNPAID_BG + ') перестала быть приглушённой: ' + round2(toGap) + ':1 к грунту');
...
test('граница не принесла ни одного нового кегля и ни одной новой краски', () => {
  ...
  for (const c of [GAP_BG, UNPAID_EDGE]) {
    const [r, g, b] = rgb(c);
  ...
  // Рамка окна: оттенок разрешён, громкость — нет. Бирюза действия (#167873)
  // даёт на белом 5.9:1, зелёный флаг результата (#047857) — 5.5:1. Рамка
  // обязана остаться волоском, иначе она встанет с ними в один ряд.
  {
    const frameOnWhite = contrast(CARD_EDGE, '#ffffff');
    assert.ok(frameOnWhite <= 2.2,
      'рамка окна (' + CARD_EDGE + ') звучит в полный голос: ' + round2(frameOnWhite)
      + ':1 на белом — это уже не контур, а сигнал наравне с кнопкой и флагом');
    assert.ok(frameOnWhite >= 1.35,
      'рамка окна (' + CARD_EDGE + ') стала волоском: ' + round2(frameOnWhite)
      + ':1 — на белом грунте границы окна больше нет');
  }
});
