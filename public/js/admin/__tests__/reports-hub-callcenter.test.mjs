...
test('построитель берёт RPC и рисовалку из определения отчёта', () => {
  ...
  // BUILDING_REPORTS_V1 — рисует в СВОЙ контейнер, а не прямо в previewEl:
  // рисовалки начинают с clear(el), и полоса разреза по зданиям над ними
  // иначе стиралась бы при каждой перерисовке. Проверяемое здесь не изменилось:
  // рисовалка по-прежнему берётся из определения отчёта.
  assert.match(hub, /\(rep\.render \|\| renderOwnerCharts\)\(body/, 'рисовалка — из определения');
});
