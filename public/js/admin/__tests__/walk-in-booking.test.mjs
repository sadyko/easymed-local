...
test('СБОЙ ВСТАВКИ СТРОКИ: визит остаётся без счёта, ошибка уходит наружу И НЕСЁТ УЖЕ ЗАПИСАННОЕ', async () => {
  seed();
  ...
  await assert.rejects(
    ...
    (e) => {
      ...
      // ЧТО УЖЕ ЛЕЖИТ В БАЗЕ — В САМОЙ ОШИБКЕ. Без этого экран знает только
      // «не получилось» и предлагает нажать ещё раз, а второй вызов
      // переиспользует визит дня и допишет В НЕГО те же строки: пациент с
      // задвоенными услугами разбирается уже в кассе. Ошибка обязана сказать,
      // что визит есть и одна строка в нём уже стоит.
      assert.ok(e.partial, 'ошибка молчит о том, что уже записано');
      assert.ok(e.partial.visit && e.partial.visit.id, 'в ошибке нет заведённого визита');
      assert.equal(e.partial.lines.length, 1, 'ошибка неверно считает уже вставленные строки');
      assert.equal(e.partial.lines[0].serviceId, CONSULT);
      assert.ok(e.partial.lines[0].visitServiceId, 'у уже вставленной строки нет её id');
      return true;
    },
  );
