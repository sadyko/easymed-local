...
// CRM_REAL_BOOKING_V1 (2026-09-21) — ЗАПИСЬ ЭТО НЕ ПРИХОД, И ЭКРАН ЕГО НЕ
// ОБЪЯВЛЯЕТ.
//
// Здесь проверялось обратное: после записи строки заявки ОБЯЗАНЫ закрыться,
// иначе ночная автоматика унесёт пришедшего в «Не пришёл». Обе половины этого
// довода больше не верны.
//
// Владелец (2026-09-21): «Пришёл» значит, что пациент ФИЗИЧЕСКИ пришёл. Запись
// из календаря — это будущий приём, часто на следующую неделю; закрыв строку
// здесь, экран объявлял бы конверсией собственное нажатие. Доказательств
// прихода три, и все три видит сервер: отметка «пришёл», деньги по счёту
// визита и начатая над пациентом работа (server/services/crm/visit-status.js).
// А сметание «Не пришёл» записанного больше и не трогает: у его строки есть
// visit_id, и crm.js такие заявки из ночной выборки исключает.
test('запись НЕ закрывает строки заявки: приход объявляет сервер, а не это окно', async () => {
    const box = await openFromCalendar();
    addBtnFor(box, 'Приём терапевта').click();
    await settle();
    await attachPatientViaUi(box);
    assert.ok(lineSelects().length, 'подстановка не отработала — проверять нечего');
    ...
    assert.equal(done, undefined,
        'окно объявило заявку дошедшей в миг записи — пациент ещё дома, а воронка уже считает его конверсией: '
        + JSON.stringify(done && done.filters));
    assert.ok(!CALLS.some((c) => c.table === 'crm_requests' && c.op === 'update'),
        'окно двигает саму заявку в обход сервера — два писателя дают ей две разные истории');
});
...
test('убранная из сметы услуга остаётся ждать: закрываются только записанные строки', async () => {
    ...
    // CRM_REAL_BOOKING_V1 — с 2026-09-21 экран не закрывает строк ВООБЩЕ; эта
    // проверка держит правило с другой стороны: убранная из сметы услуга и
    // подавно должна остаться ждать своего дня.
});
