...
//
// DIAL_LINE_V1 — на экране появился ещё один список («с какой линии звонить»),
// и он стоит ВЫШЕ маршрута. Брать все списки подряд теперь нельзя: маршрут
// узнаётся по своей пометке, а не по месту в DOM — иначе следующая настройка со
// списком снова сдвинет эти тесты.
const routeSelects = (root) => findSelects(root).filter((n) => 'data-route-select' in (n.attrs || {}));
...
test('501 только на маршруте — остальной экран живёт, карточка говорит «недоступен»', async () => {
  ...
  // Речь о списках МАРШРУТА: наполнить их нечем, значит их и нет. Выбор линии
  // для исходящих (DIAL_LINE_V1) — своя настройка, она от маршрута не зависит и
  // при его отказе остаётся на экране.
  assert.strictEqual(routeSelects(root).length, 0, 'ни одного списка, который нечем наполнить');
});
