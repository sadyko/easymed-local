...
// МАСТЕР — ДВЕРЬ ПРИВЯЗКИ, А НЕ ТОЛЬКО ЗАПИСИ (разбор ревью, R1). «Направить
// на услуги» из кабинета врача и «анализы утром, консультация после обеда» у
// стойки — это вторая услуга в ТОТ ЖЕ день, а визит у пациента на день один
// (и календарь второго тоже не заведёт). Отказ по занятому дню оставлял
// такого пациента без строк, без счёта и без маршрутного листа. Поэтому
// здесь занятый день — не отказ: услуги ложатся в существующий приём со
// своим scheduled_at, головной слот не занимается, и об этом сказано вслух.
test('МАСТЕР: визит дня с работой — услуги ЛОЖАТСЯ в него, слот не занимается, и об этом сказано', async () => {
  ...
  try {
    ...
    assert.equal(DB.prepare('SELECT COUNT(*) c FROM visit_services WHERE visit_id = 55').get().c, 1,
      'услуга не легла в существующий приём — пациент у стойки остался без строки, счёта и маршрутного листа');
    assert.ok(!RPC.some((c) => c.name === 'calendar_book'),
      'мастер занял слот вторым вызовом, хотя сервер сказал, что время визита не меняется');
    assert.ok(TOASTS.some((t) => /11:20/.test(t) && /Петров Пётр/.test(t) && /время не занято/.test(t)
        && /услуги записаны на существующий приём/i.test(t)),
      'регистратору не сказано, что слот не занят, а услуги легли в уже идущий приём: ' + JSON.stringify(TOASTS));
    assert.equal(w.saved(), 1, 'onSaved не вызван — кабинет врача не получит маршрутный лист');
