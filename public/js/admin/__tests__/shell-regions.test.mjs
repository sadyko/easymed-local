...
test('в верхней строке имя раздела слева и все пять органов управления справа', () => {
    ...
    // APPBAR_BACK_V1 (2026-09-06) — ПОРЯДОК ИЗМЕНЁН ОСОЗНАННО, и вот чем это
    // оправдано.
    //
    // Здесь стояло «имя раздела обязано стоять ПЕРВЫМ в строке»: глаз ищет
    // его первым. Правило верное, и оно НЕ НАРУШЕНО — потому что кнопка
    // возврата по умолчанию скрыта (hidden) и появляется только на подэкранах,
    // на которые из бокового меню не попасть. На всех восемнадцати экранах
    // меню имя по-прежнему первое и единственное.
    //
    // А на подэкране первым нужен путь наружу: «‹ Настройки | Компания»
    // читается как хлебная крошка — где я нахожусь и внутри чего. Обратный
    // порядок («Компания | ‹ Настройки») читается задом наперёд, а кнопку
    // «назад» справа от заголовка не ищет никто.
    assert.equal(BAR_NODE.children[0].attrs.id, 'section-back',
        'кнопка возврата пропала из верхней строки');
    assert.ok('hidden' in BAR_NODE.children[0].attrs,
        'кнопка возврата видна по умолчанию — на экране из меню возвращаться некуда, и она бы там только мешала');
    assert.equal(BAR_NODE.children[1], title,
        'имя раздела обязано идти сразу за кнопкой возврата и до всего остального');

    const controls = BAR_NODE.children[2];
...
test('верхнюю строку не может перекрасить второй лист стилей', () => {
    ...
    // WHITE_GROUND_GLOW_V1 — запрет на белый снят намеренно: грунт страницы
    // сам стал белым. Правило, которое эта строка охраняла, целиком выражено
    // следующей: у верхней строки НЕТ своей подложки, она лежит на грунте —
    // каким бы он ни был. Ниже к нему добавлен запрет на собственную тень:
    // именно тень (а не цвет) сделала бы из неё отдельную поверхность.
    assert.equal(hex(bg.value), hex(GROUND), 'верхняя строка лежит не на грунте страницы, а на своей подложке');
    const barShadow = winner('box-shadow');
    assert.ok(!barShadow || /^(none|0)/.test(barShadow.value),
        'у верхней строки появилась тень (' + (barShadow && barShadow.value) + ') — это снова отдельная поверхность');
