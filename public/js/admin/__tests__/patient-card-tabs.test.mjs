...
test('уровень «Просмотр» не показывает кнопок правки, «Изменение» — кнопок удаления', async () => {
  ...
  assert.ok(!titles(box).includes('Удалить'), '«Изменение» без «Удаления» показывает удаление документа');
  ...
  openTab(box, 'Документы');
  assert.ok(titles(box).includes('Удалить'), '«Удаление» не дало удалить документ');
});
