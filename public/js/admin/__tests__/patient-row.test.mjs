...
test('FAST_REGISTRATION_V1: на месте поиска по дате рождения — ОДНА пульсирующая кнопка, и она открывает быструю регистрацию', async () => {
  ...
  // Клик открывает ТО ЖЕ окно заведения пациента, но в быстром режиме: и имя
  // окна, и главное действие в подвале говорят про услуги.
  document.body.children.length = 0;
  btn.click();
  const dlg = walk(document.body).find((n) => n.attrs['data-dialog'] === 'patient-create');
  assert.ok(dlg, 'кнопка не открыла окно заведения пациента');
  const inside = textOf(dlg);
  assert.ok(inside.includes('Быстрая регистрация'),
    'окно открылось в обычном режиме, а не быстром');
  assert.ok(inside.includes('Сохранить и добавить услуги'),
    'в быстром режиме нет главного действия «Сохранить и добавить услуги»');
});
