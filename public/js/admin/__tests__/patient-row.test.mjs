...
test('FAST_REGISTRATION_V1: на месте поиска по дате рождения — ОДНА пульсирующая кнопка, и она открывает быструю регистрацию', async () => {
  ...
  // Реквизиты пациента — те же разделы сборщика полей, что и в окне заведения.
  assert.ok(inside.includes('Личные данные'), 'в окне нет реквизитов пациента: ' + inside.slice(0, 200));
