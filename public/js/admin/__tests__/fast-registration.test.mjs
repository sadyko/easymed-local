...
// PICKER_CATALOG_EVERYWHERE_V1 — «+Услуги» открывает КАТАЛОГ, а не три колонки.
// Владелец, впервые открыв это окно: «i see an old version with 3 columns».
// У окна выбора услуги два облика в одном файле, и без attachMode оно открывает
// старый каскад «Группы услуг → Услуги → Врачи». Второй флаг не менее важен:
// requireSlot: false снимает требование выбрать врача и время на каждой строке —
// пришедшему без записи слот не нужен, а врача регистратор выбирает здесь же, в
// строке таблицы. Проверяется исходником — как и соседние пины выше: каталог
// открывается отдельным окном, и вызов виден только в тексте вызова.
// Поведение самого каталога с этими флагами проверяет service-picker-attach.test.mjs.
test('«+Услуги» зовёт каталог в режиме привязки и без обязательного слота (иначе открываются старые три колонки)', () => {
  const src = srcOf('views/fast-registration.js');
  const call = src.match(/openServicePickerModal\(\{[\s\S]*?\}\);/);
  assert.ok(call, 'вызов каталога услуг в окне регистрации не найден');
  assert.match(call[0], /attachMode:\s*true/,
    'каталог зовут без attachMode — откроется старый каскад из трёх колонок');
  assert.match(call[0], /requireSlot:\s*false/,
    'каталог зовут с обязательным слотом — «Готово» останется серым, пока не выберут врача и время');
});
