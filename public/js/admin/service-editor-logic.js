...
// «То же имя» = совпадение после trim и без учёта регистра — ровно правило
// усыновления из branch-sync/catalogue.js (его norm). Сознательно НЕ SQL
// LOWER(): SQLite не складывает регистр кириллицы, а JS toLowerCase —
// складывает, поэтому сравнение живёт здесь и выполняется на строках из базы.
