...
// SERVICE_CAT_BY_TYPE_V1 (2026-09-14) — раздел = ТИП услуги, тот самый, что
// стоит в редакторе и в списке «Услуги» (services.type, пять значений). Раньше
// раздел угадывался по НАЗВАНИЮ русскими словами («консульт», «узи», «операц»…),
// и всё, что названо по-узбекски или по-английски — «NUGA BEST», «BEL UMURTQA
// CHECK UP», коды анализов — падало в «Прочее»: 341 услуга из 543 у владельца
// («we have only 5 types… why is there other: 341»). Название читается только
// у строки без типа (старый импорт); 'radiology' — устаревшее значение, идёт
// в «Диагностику» (SERVICE_TYPES_FIVE_V1).
const TYPE_NAME = {
    consultation: 'Консультации',
    lab:          'Лаборатория',
    procedure:    'Процедуры',
    imaging:      'Диагностика',
    radiology:    'Диагностика',
    other:        'Хирургия',   // в базе хирургия хранится как 'other' — см. миграцию 109
};

// Раздел услуги: по типу; у строки без типа — по признаку лаборатории, дальше
// по названию.
export function categoryOf(s) {
    if (!s) return 'Прочее';
    const t = String(s.type || '').trim().toLowerCase();
    if (t && TYPE_NAME[t]) return TYPE_NAME[t];
