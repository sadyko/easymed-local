...
**ЧТО НЕ СОШЛОСЬ И ПОЧЕМУ (не «доделать потом», а решение).** Две правки ОДНОЙ
колонки на двух узлах в один такт не сходятся: каждый отказывает другому по
...
Лечится это одним из двух, и оба — отдельная задача, а не строчка:
- сравнивать по ВРЕМЕНИ ПРАВКИ на проводе: везти `edited_at` рядом с меткой и
  хранить его в `sync_seen`. Тогда обе стороны выбирают одного победителя.
  Цена: узел с отставшими часами проигрывает всегда — ровно от этого HLC и
  поднимает пол, так что менять придётся обе половины сразу;
- не подтверждать колонки, которые мы отвергли, — но подтверждение у нас на
  СРЕЗ, а не на колонку, и «не подтвердить» значит повторять срез вечно.

До тех пор одноимённая правка в двух зданиях в один час — известная граница
фазы. Для регистратуры это редкость (телефон правят там, где пациент стоит), а
последствие видно глазами: два разных значения в двух филиалах, не потеря.
