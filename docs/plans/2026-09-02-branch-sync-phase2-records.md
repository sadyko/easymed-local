...
**Что НЕ закрыто.** Курсор холодного засева (`seed_*`) по-прежнему двигается по
выгрузке: все его страницы несут один и тот же `upto`, и по квитанции их не
различить, поэтому пропущенная страница ЗАСЕВА не повторяется. Это окно
первого знакомства с соседом, а не ежедневная работа; закрывать его надо
номером страницы в квитанции — отдельной задачей. Второе: сосед, который
принимает, но сам не выкладывается, не шлёт квитанций и держит журнал
отправителя целиком, пока не станет STALE_DAYS-молчуном и не будет забыт.
