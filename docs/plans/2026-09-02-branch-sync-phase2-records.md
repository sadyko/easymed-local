...
**Что было сломано.** Блоб узла на релее ОДИН и замещается следующей выгрузкой,
а отметка «отдано» ставилась по ответу сервера 2xx. Узел, не забравший блоб до
следующей выгрузки соседа (компьютер выключен на ночь, канал лёг на час), её
содержимое не получал НИКОГДА: отправитель второй раз его не слал. Терять
вечерние анализы филиала молча нельзя, поэтому Фаза 2 без этой задачи в живую
сеть не выкатывается.

**Два горизонта на соседа** (`sync_peers`, миграция 084 — не выпущена, правится
на месте). Одной отметкой не обойтись: пробная сборка Задачи 7 двигала по
квитанции общий `sent_seq`, а его же читает защита местной неотправленной
правки — и на задержке подтверждения B не принимал адрес от C, C не принимал
телефон от B, ни один не подтверждал, и сеть не сходилась вовсе.

- `pub_seq` — докуда **выложено** (двигает `markPublished` по 2xx). Снимает
  защиту местной правки: `localUnshippedCols` читает теперь его. Обоснование —
  защита закрывала ровно одно окно, «нашего авторства ещё не видно соседу»;
  как только блоб лёг на сервер, окно закрыто, а спор двух настоящих правок
  разбирают поколоночные метки `sync_seen`.
- `sent_seq` — докуда **подтвердил сосед** (двигает `markConfirmed` по
  квитанции). По нему собирается срез и чистится журнал, поэтому
  неподтверждённая запись лежит в КАЖДОМ следующем блобе, пока квитанция не
  придёт. `markSent` остался тонким синонимом «оба сразу» — для прямого
  обмена (Маршрут А), где 2xx и означает «сосед применил».

**Накопительный срез.** `buildBatch` читает хвост от `sent_seq`. Предел —
страница: если неподтверждённого больше `limit`, уезжают самые старые `limit`
строк, остальные следующими выгрузками; не теряется ничего, потому что
`sent_seq` стоит. Заголовок среза несёт `upto` — номер журнала отправителя, до
которого срез доходит.

**Квитанции едут в блобе**, отдельного канала нет:
`{ v: 1, from: 'B', acks: { A: 4120 }, batches: { A: { records, upto, seed? } } }`.
`acks[буква]` — докуда мы применили журнал ЭТОГО соседа (`sync_peers.recv_upto`,
пишется ТОЙ ЖЕ транзакцией, что и записи). `fetchJournals`, открыв чужой блоб,
первым делом читает `acks[себя]` и зовёт `markConfirmed`; квитанция
ограничивается `pub_seq` — она приезжает снаружи, и подтвердить больше
выложенного не даём. Узел, которому нечего слать, выкладывается всё равно,
если квитанции изменились: иначе сосед сутки повторял бы уже применённое.

**Идемпотентность (`recv_upto`).** Срез с `upto` не больше `recv_upto` уже
разобран — он пропускается целиком, ДО резервной копии (иначе повтор, который
сосед кладёт в каждый блоб, заваливал бы диск копиями). Страницы холодного
засева под правило не подпадают: у всех у них `upto` один и тот же —
замороженный пол журнала.

**Авторство сужается по квитанции (`changed_at`).** Найдено при реализации и
воспроизведено на двух базах: в повторе едет и авторство СТАРОЙ записи, а у
вставки авторство — `'*'`, вся строка. Сосед, успевший поправить у себя адрес и
уже его выложивший (то есть незащищённый), получал наш устаревший снимок под
свежей меткой и терял правку НАВСЕГДА — обе базы расходились и не сходились
больше никогда. Поэтому запись несёт `changed_at` — «колонка → номер журнала её
последней правки», — и приёмник отбрасывает притязания не выше своего
`recv_upto`. `changed` остаётся списком: узел старой сборки читает его, как
читал.

**Страницы засева — по номеру.** Все страницы засева несут один и тот же
`upto` (замороженный пол), различить их квитанцией нечем, поэтому у страницы
есть свой номер: `seed_page` в срезе, `recv_seed_page` у приёмника, и он же во
второй половине квитанции. Курсор (`seed_tbl/seed_at/seed_id`) — это
ПОДТВЕРЖДЁННОЕ место: `buildBatch` собирает страницу от него, `markPublished`
его не трогает (кладёт конец страницы в `seed_next_*`), переносит только
`markConfirmed` с номером этой страницы. Незабранная страница уезжает снова.
Без этого у клиники на 70 000 пациентов засев рвался посередине, и дыра
приходилась на старых пациентов, которых никто не трогает, — то есть была
невидимой.

Две вещи, без которых это не работает, обе найдены на тестах:

- **Набор строк заморожен** (`seed_started`). Порядок засева детерминирован, а
  набор — нет: пока сосед молчал, клиника завела ещё сорок пациентов, и
  «страница 1» во второй выгрузке — уже другие строки. Тогда сосед, отсеивающий
  повтор по номеру, этих сорока не увидел бы, а применяющий заново — получил бы
  снимок с авторством `'*'` под свежей меткой и потерял бы СВОИ правки
  (сквозной тест: у филиала пропадали и телефон, и адрес). Засев отдаёт то, что
  существовало на его начало; заведённое позже везёт журнал — его seq выше пола.
  Время сверяется в формате `created_at` (секунды): `toISOString()` с
  миллисекундами делает сравнение `'…22Z' <= '…22.563Z'` ложным, и из засева
  выпадала бы вся его первая секунда.
- **Пустая последняя страница закрывает засев сразу.** Пустой срез в блоб не
  кладётся вовсе, сосед его не видит и подтвердить не может — узел, впервые
  выложившийся пустым (клиника, где ещё никого не завели), остался бы «в
  засеве» навсегда, и первый же заведённый пациент не уехал бы никуда.

**Ожидание ограничено `last_ack`.** «Забыть молчуна» решал `last_ok`, но он
обновляется на КАЖДОЙ нашей удачной выгрузке: у соседа, выключенного навсегда,
он всё равно свежий. Пока горизонт был один, это ничего не значило; теперь
такой сосед держал бы и свой недосеянный засев, и чистку нашего журнала вечно.
`last_ack` ставится при заведении строки (фора новому соседу) и обновляется
каждой пришедшей квитанцией; STALE_DAYS считаются по нему.

**Что НЕ закрыто.**

- Сосед, который принимает, но сам не выкладывается, квитанций не шлёт и держит
  журнал отправителя целиком до STALE_DAYS. Это принято сознательно: узел,
  который не выкладывается, в сети и так неисправен, а 30 дней журнала —
  цена, которую клиника переживёт (счёт идёт на десятки мегабайт).
- Явно заданный `limit` выгрузки больше не поднимается молча до
  `MIN_JOURNAL_LIMIT` (ревью 7/7b, I3): с ним сквозной тест «засев страницами»
  отдавал всё ОДНОЙ выгрузкой и постраничность не проверял вовсе. Теперь
  `MIN_JOURNAL_LIMIT` — дно ДЕЛЕНИЯ пополам при слишком большом блобе, а не
  запрет на маленькую страницу.

**Найдено попутно, НЕ этой задачей внесено и НЕ ею закрыто.** Холодный засев
отдаёт строку с авторством `'*'` и СВЕЖЕЙ меткой, а снимок в нём — копия
отправителя. Значит узел, засеянный третьим филиалом ПОЗЖЕ, может получить от
него устаревшее чужое значение поверх более свежего, которое у него уже было:
B правит адрес и засевает им C, а A, про эту правку ещё не знающий, засевает C
следом — и адрес у C откатывается. Проверено на обеих сборках, ДО и ПОСЛЕ
Задачи 7b, поведение дословно одинаковое. Мерить это стоит так: прогон 40
случайных сетей из трёх узлов, где каждый правит СВОЮ колонку, а выгрузки и
выборки случайно пропускаются, — до Задачи 7b работа терялась в 31 прогоне из
40, после — в 1, и этот один как раз про засев. Лечится тем же, чем и
пропущенная страница засева: авторством по колонкам вместо `'*'`.

**Тесты.** `journal.test.js` — выложенное без подтверждения уезжает ещё раз;
чистка ждёт квитанции; квитанция больше выложенного обрезается.
`records.test.js` — выгрузка снимает защиту (дальше спорят метки); повтор среза
не делает ничего; сужение авторства спасает свежую правку. `records-e2e.test.js`
— тест «ОГРАНИЧЕНИЕ: пропустивший выгрузку теряет» ПЕРЕВЁРНУТ: теперь
пропущенное доезжает со следующим блобом, квитанция возвращается к отправителю,
повторная выборка того же блоба не применяет ничего и не снимает копию.

---

## Задача 7c: сходимость одноимённых правок и мелочи ревью

**C1 — корень.** Местная правка не имела метки ВООБЩЕ: `sync_seen` помнит
только ПРИНЯТЫЕ метки, то есть сравнение «кто новее» шло чужая-против-чужой, а
своя держалась одной защитой — а защита снимается выгрузкой. Отсюда:
B правит телефон в 09:00 и выкладывается; A этого не забирал и правит тот же
телефон в 09:50; в 10:00 такт A сперва ВЫКЛАДЫВАЕТ (защита снимается в тот же
миг), потом ЗАБИРАЕТ вчерашний блоб B и кладёт 09:00 поверх 09:50. Откат в
журнал не пишется, B уже применил значение A — расхождение навсегда.
Перестановкой выгрузки и выборки это не лечится.

**Правило.** У местной правки теперь есть метка: журнал помнит время каждой
правки и какие колонки менялись (`cols`), и метка чеканится ТЕМ ЖЕ путём, каким
её отдаст `buildBatch` — `nextStamp` от того же `at` и тех же сохранённых часов.
Приехавшая колонка, проигравшая этой метке, не применяется; выигравшая —
применяется, как и раньше. Отдельного счётчика нет: это `skipped`.

**Что при этом выяснилось про сами метки, и это важнее правила.** Часы HLC
поднимаются до `Date.now()` при КАЖДОМ приёме (`writeClock`), поэтому метка,
которую узел чеканит выгрузкой, — это его время ОТПРАВКИ, а не правки, вопреки
комментарию в `buildBatch`. Замерено: метка правки двухчасовой давности до
первого приёма несёт её настоящее время, после первого приёма — «сейчас».
Значит сравнение здесь на деле такое: «наша неотправленная правка против
записи, отчеканенной соседом ДО нашего последнего обмена». Для дефекта C1
этого достаточно (узлы синхронизируются раз в час), и сквозной тест
воспроизводит сценарий целиком, выставляя часы явно.

**Пробовал и откатил** (прогон 40 случайных сетей из трёх узлов, каждый правит
ОДНУ И ТУ ЖЕ колонку, выгрузки и выборки случайно пропускаются):

| вариант | расхождений из 40 |
| --- | --- |
| до 7c | 24 |
| чеканить метку от ТЕКУЩИХ часов («любая неподтверждённая правка сильнее») | 26 |
| запоминать выигравшую местную метку в `sync_seen` | 19 |
| **принято: метка от `at` и сохранённых часов** | **16** |

Второй вариант хуже потому, что узлы начинают отказывать друг другу
симметрично. Третий — потому что отказ по живой журнальной записи безопасен
(она ещё не подтверждена, значит ещё уедет, и сосед придёт к тому же значению),
а отказ по метке в `sync_seen` переживает чистку журнала и делается от имени
правки, которую больше никто не пришлёт.

**ЧТО НЕ СОШЛОСЬ ТОГДА — И ЗАКРЫТО ЗАДАЧЕЙ 7d.** Две правки ОДНОЙ колонки на
двух узлах в один такт не сходились: каждый отказывает другому по
своей неподтверждённой правке, оба при этом подтверждают ПРИЁМ среза (иначе
срез повторялся бы вечно), и после подтверждения ни один из них уже не пришлёт
своё значение снова — расхождение застывает. 16 прогонов из 40. Правки РАЗНЫХ
колонок (то, ради чего фаза и делалась) сходятся: 1 из 40, и тот единственный —
про засев, см. ниже.

Выбран первый из двух путей — сравнивать по ВРЕМЕНИ ПРАВКИ, с меткой на
колонку (Задача 7d ниже); второй («не подтверждать отвергнутые колонки»)
отброшен: подтверждение у нас на СРЕЗ, а не на колонку, и «не подтвердить»
значит повторять срез вечно.

**Заодно, по тому же ревью:**
- **I1** — `pruneBackupsByKind` вызывается сразу за копией безопасности.
  Копия снимается на каждый обмен, где есть что применять (десятки в сутки), а
  чистка шла только при старте и внутри суточной копии.
- **I2** — отказ базы больше не молчит. Новая таблица `sync_refused`
  (`tbl, uid, peer, err, at`) пишется ТОЙ ЖЕ транзакцией; в сводке появился
  счётчик `refused` отдельно от `skipped`, а на экране — причина
  `records_refused`. Квитанция при этом уезжает по-прежнему: иначе одна
  «ядовитая» строка повторялась бы в каждом блобе вечно.
- **I3** — явный `limit` выгрузки больше не поднимается до `MIN_JOURNAL_LIMIT`
  (сделано вместе со страницами засева).
- **M1** — запись, у которой `origin` и буква в метке называют РАЗНЫЕ узлы, не
  применяется вовсе: одна решает, чью защиту снимать, вторая — чьей буквой
  подписать строку, и разойтись им нельзя.
- **M2** — приём заводит строку `sync_peers`, если её нет (иначе квитанция
  соседу, выложившемуся первым, не записывалась). «Холодный» для `buildBatch`
  теперь — «строки нет ИЛИ `last_ok IS NULL`»: иначе первая же принятая порция
  объявляла бы соседа тёплым, и наши строки не уехали бы к нему никогда.
- **M3** — `backup_failed` больше не обрывает обмен `break`-ом: причина
  сообщается ПО СОСЕДУ, остальные продолжают.
- **M4** — переименование филиала в списке сети сверяет букву той же
  `COLLATE NOCASE`, что и поиск строки; разъехавшись, они молча оставляли
  филиал под старым именем.
- **M5** — часовой обмен записями на главной клинике совмещается сам с собой
  (`exchangeInFlight`), как кнопка «Синхронизация» давно совмещается через
  `inFlight`.
- **Порт в сквозных тестах** — `listen()` в `records-e2e` и `relay-e2e` заменён
  общим помощником (`control-plane/server/test-helpers/listen.js`), а
  `CLOSED_MAIN` переехал с порта 1 на 65535: порт 1 сам стоит в списке WHATWG
  «bad ports», и `fetch()` отказывал на нём ДО попытки соединения — то есть
  тест «прямой путь закрыт» проверял не тот отказ, который описывает.

---

## Задача 7d: метка авторства — время правки, не время отправки

**Находка, из которой всё следует.** Часы HLC поднимают свой пол до
`Date.now()` при КАЖДОМ приёме (`writeClock`), поэтому метка, которую узел
чеканит выгрузкой, — это его время ОТПРАВКИ, а не правки, вопреки комментарию в
`buildBatch`; замерено прямо: до первого приёма правка двухчасовой давности
несёт честные «два часа назад», после первого — «сейчас». Пока метка решала
только порядок приёма, это ничему не мешало, но как только по ней стали
разбирать, чья правка колонки новее, побеждать начал не тот, кто правил позже,
а тот, кто позже вышел на связь.

**Правило.** У каждой авторской колонки своя метка, отчеканенная от ВРЕМЕНИ ЕЁ
ПРАВКИ и без пола часов: `stampAt(ms, node)` — тот же формат и тот же
компаратор, счётчик всегда ноль, тай-брейк — буква узла. Запись несёт
`stamps: {колонка: метка}`; `stamp` записи остаётся для порядка, `sync_origin`
и надгробий, `changed_at` — как было. Приёмник сравнивает метку колонки с
`sync_seen` и с меткой МЕСТНОЙ правки той же колонки (журнал знает её время),
и в `sync_seen` кладёт метку колонки, а не записи. Обе стороны сравнивают ОДНИ
И ТЕ ЖЕ две строки — отсюда и сходимость: победитель у них получается один.
Проигравшая сторона ничего не теряет, её правка не подтверждена и ещё уедет.

**Часы HLC остались** — за причинностью и наблюдением за перекосом; арбитром
авторства они быть перестали.

**Точность журнала — миллисекунды** (`%f` в 084). С секундной точностью две
правки одной колонки внутри одной секунды неразличимы, и вторая молча
пропадала: приёмник видел метку, равную уже принятой, и запись отбрасывал.

**Засев отдаёт чужое авторство как чужое** (это и закрывает gap iii). Свои
колонки подписываются временем строки (`updated_at`, где он есть, иначе
`created_at`, и НИКОГДА не «сейчас»); колонки, принятые от третьего филиала, —
их собственной меткой из `sync_seen`. Надгробие несёт время удаления, поэтому
«удаление проигрывает более поздней правке» считается на той же оси.

**Цена названа вслух: это сравнение НАСТЕННЫХ часов двух зданий.** Спецификация
выбирает победителем последнюю ПРАВКУ, значит филиал с отставшими часами будет
проигрывать спор за одну и ту же колонку. Часы клиник ходят по NTP; расхождение
в минуты стоит того, что при одновременной правке одного поля в двух зданиях
выживет не то значение — но выживет ОДНО и то же в обоих зданиях, а это и есть
разница между «спорным значением» и «разъехавшимися базами».

**Замер (40 случайных сетей из трёх узлов, выгрузки и выборки случайно
пропускаются):**

| | до 7b | после 7c | после 7d |
| --- | --- | --- | --- |
| у каждого узла своя колонка | 31 из 40 | 1 из 40 | **0 из 40** |
| все правят одну колонку | 24 из 40 | 16 из 40 | **0 из 40** |

Прогон, расходившийся после 7c из-за позднего засева, тоже сошёлся: метки
колонок в засеве теперь настоящие.

---

## Задача 7e: авторство переживает чистку; защита уступает меткам; часы; темп засева

**C-1. Авторство жило в журнале, а журнал чистится.** Один обычный сбой: у
филиала PUT отвечает 500, GET работает. B забирает срез A, не применяет из него
ничего (правка A новее), но квитанцию пишет; A читает её, двигает `sent_seq`,
`pruneJournal` сносит журнальную строку — и метка правки A исчезает вместе с
ней. Дальше B присылает своё СТАРОЕ значение, A его принимает: сеть сходится на
более ранней правке, оба журнала пусты, искать нечего.

Авторство переехало в отдельную таблицу `sync_authored(tbl, uid, col, at)`,
которую пишут те же журнальные триггеры (обойти нельзя) и не трогает чистка.
Вставка туда не пишет — полом служит `created_at` строки; удаление строки
уносит и её авторство. Приём авторства НЕ пишет: триггеры не отличают приём от
здешней правки, поэтому `applyBatch` снимает снимок до записи и возвращает его
в конце транзакции — ровно как он уже делал с журналом. Без этого узел объявлял
своей каждую принятую колонку, да ещё и временем «сейчас»: в прогоне сети
правки переставали расходиться вовсе (40 из 40).

**I-1 попутно закрыт тем же.** `updated_at` — один на строку, и как время
правки КОЛОНКИ он врал: правка заметки старила адрес, а сосед потом держал
устаревший адрес вечно. У `visit_services` и `lab_results` его нет вовсе.
Теперь: свои колонки — из `sync_authored`, принятые — из `sync_seen`, остальные
— пол `created_at`. Никогда не «сейчас» и никогда не «последнее касание».

**Пол засева для ПРИЕХАВШЕЙ строки** — отдельная находка того же прогона. У
такой строки `created_at` означает не «когда её завели», а «когда МЫ её
приняли», то есть заведомо позже правки, которая её привезла. Ставя его,
узел выдавал чужому значению метку новее всего на свете и замораживал колонку
по всей сети. Теперь пол для приехавшей строки — самая РАННЯЯ известная нам
метка этой строки: ниже неё мы всё равно ничего не знаем, а низкая метка
безопасна (сосед без своих данных её возьмёт, сосед с новее — оставит своё).

**C-1(b). Защита снята целиком.** `localUnshippedCols`/`pub_seq`-защита была
нужна, пока у местной правки не было метки. Теперь она не только лишняя, но и
вредна: наша правка СТАРШЕ приехавшей — защита оставляла бы наше устаревшее
значение, а сосед, приняв нашу правку и увидев её младше своей, оставил бы
своё; две базы расходятся на ровном месте. Плюс квитанция уезжала и на
«защищённую» запись. `pub_seq` остался как показатель «докуда выложено» и как
потолок для квитанции. Замерено: и с защитой, и без неё все четыре варианта
прогона дают 0 расхождений из 40 — выбран вариант БЕЗ неё, как более простой и
более верный.

**I-2. Часы.** Метка решает, чья правка новее, поэтому компьютер с часами из
будущего раздаёт всей сети метки, которых не обгонит ни одна честная правка:
поле застывает у всех. На приёме метка каждой колонки подрезается до
«сейчас + 5 минут» ДО сравнения и до записи в `sync_seen` — подрезается, а не
отбрасывается: в записи настоящая работа филиала. Перекос считается,
запоминается в `sync_peers.clock_skew_ms` и выходит на экран причиной
`peer_clock_skew` («Часы филиала B спешат на 3 ч — проверьте время»); если
спешат ВСЕ соседи сразу, показывается зеркальная `local_clock_behind`.
Заморозка после этого ограничена допуском, а не годом.

**I-3. Темп засева.** Страница подтверждается соседом, а такт был часовой:
выложили — забрали — подтвердили, три часа на страницу. Клиника на 70 000
пациентов — это около 1,33 млн строк (визиты, услуги, результаты), то есть 266
страниц по 5000: **33 дня**. Теперь страница засева — 20 000 строк (`SEED_LIMIT`,
по-прежнему делится пополам, если блоб не влезает), а пока идёт засев, оба узла
тикают раз в 10 минут вместо часа: 67 страниц × 3 такта × 10 мин ≈ **34 часа**,
то есть полтора суток. Если блоб придётся делить пополам до 5000 строк, выйдет
около 5,5 суток — всё равно вшестеро быстрее прежнего. Экран больше не молчит:
`branchSyncStatus` отдаёт `seed`, филиал показывает «Первичная загрузка из
филиала B: страница 3 из ~12», главная — «Идёт первичная загрузка в филиалы: B,
C (страница 3)».

**Мелочи того же ревью.** Общий замок обмена на оба входа (часы и кнопка) —
`withExchangeLock` в relay.js, вместо двух флагов, которые друг друга не видели.
Висячий `else` в `syncRecords` разложен по порядку «сперва про канал, потом про
содержимое»: общий отказ выборки больше не молчит, когда есть отказы базы.
`sync_refused.peer` стал NOT NULL (в PRIMARY KEY NULL не склеивается, и строки
копились бы), отказ снимается, когда строка всё-таки применилась, и выселяется
через 60 дней. `sync_tombstones.at` — с миллисекундами, как журнал. В 084
вернулся перевод строки в конце файла.

**Замер (40 случайных сетей из трёх узлов; «сбой выгрузки» — PUT отвечает 5xx,
GET работает, плюс правка между выгрузкой и выборкой):**

| вариант | до 7e | после 7e |
| --- | --- | --- |
| у каждого узла своя колонка | 0 из 40 | **0 из 40** |
| все правят одну колонку | 0 из 40 | **0 из 40** |
| своя колонка + сбой выгрузки | 16 из 40 | **0 из 40** |
| одна колонка + сбой выгрузки | 10 из 40 | **0 из 40** |

Расхождений не осталось ни в одном варианте.

---

## Задача 7f: разбор финального ревью Фазы 2

**F-1. Пол засева для ПРИЕХАВШЕЙ строки падал обратно на «сейчас», как только
чистка забывала её метки.** 7e поставила полом самую РАННЮЮ известную нам метку
строки — но известные метки лежат в `sync_seen`, а `sync_seen` выселяется по
горизонту в 90 дней. После выселения известных не остаётся, пол возвращался к
`rowStamp`, то есть к нашему `created_at`, то есть к моменту, когда МЫ эту
строку приняли. Повторный засев после этого отдавал каждую колонку чужой строки
меткой «сегодня», и настоящее, более новое значение соседа проигрывало ей
навсегда — тот же дефект I-1, только через чистку.

Достать этот горизонт можно и внутри ОДНОГО холодного засева клиники с
историей: поздние страницы несут свежие метки, они двигают горизонт, и строки
2020 года выселяются из `sync_seen` раньше, чем их страницу переспросят.

Теперь пол приехавшей строки — ЭПОХА (`stampAt(0)`), метка доказуемо низкая.
Про колонку чужой строки, которой нет в `sync_seen`, мы не знаем ничего — и
говорим об этом эпохой: сосед без своих данных значение возьмёт, сосед с любым
своим оставит своё. Низкая метка безопасна, высокая — ложь. У строки,
заведённой ЗДЕСЬ, пол остаётся её `created_at`: «раньше её не было» — правда,
которую мы знаем точно.

**F-2. Фразы о сбитых часах доезжали до экрана с дырками.** Обе написаны с
подстановками (`{letter}`, `{offset}`) — иначе они не сказали бы ни какой
филиал, ни насколько, — а `reasonText` был голым поиском по словарю: владелец
прочитал бы «Часы филиала {letter} спешат на {offset}». Теперь `reasonText`
принимает вторым аргументом итог обмена (старый вызов работает дословно как
работал) и подставляет букву и смещение словами человека: «3 ч», «12 мин»,
«2 дн». Готовый текст едет в `fetch_message` — собрать его может только сервер,
у него на руках сам перекос.

Там же: перекос, который мы запоминаем и показываем, ограничен десятью годами.
Метка приезжает снаружи, её формат допускает 48 бит миллисекунд, и подделанная
`ffffffffffff` записывала в `sync_peers.clock_skew_ms` около 8,9 миллиона лет —
число, по которому ничего не сделаешь. И причина о часах больше не затирает
`records_refused`: совпадают они не случайно (сбитые часы соседа — ровно та
причина, по которой база отвергает его записи), а «часть записей не принята» —
новость тяжелее. Код остаётся за отказом, часы дописываются текстом.

**F-3. Строку о первичной загрузке никто не рисовал.** `seedLine` появилась в
7e и не вызывалась ниоткуда: сервер считал страницы (`branchSyncStatus.seed`),
экран о них молчал — ровно тот обрыв провода, ради которого написан
`branch-sync-view.test.mjs`. Теперь строка стоит вверху карточки, выше разбора
по ролям: филиал видит «Первичная загрузка из филиала B: страница 3 из ~12»,
главная — «Идёт первичная загрузка в филиалы: B, C (страница 3)». Засева нет —
строки нет вовсе: постоянное «загрузка не идёт» учит не читать это место.

**Мелочи того же ревью.** Поле `protected` в ответе `applyBatch` снято: защита
исчезла в 7e, и с тех пор счётчик был вечным нулём, который читается как
«защищать не пришлось» вместо «защиты не существует». Транзакция приёма стала
`.immediate()` — она читает (квитанцию, `sync_seen`, местные метки) и пишет по
прочитанному, а отложенная транзакция в этом положении получает
`SQLITE_BUSY_SNAPSHOT` и теряет уже сделанную работу (так же и по той же
причине это сделано в `identity.js` и `letters.js`).

**Ёмкость `sync_authored`.** Рядом с числом `sync_seen` (~190 строк на принятый
визит с панелью; ~17 млн строк, 1,4 ГБ в год у принимающего филиала) её надо
назвать вслух: `sync_authored` — это O(строк × различных ПРАВЛЕННЫХ колонок), и
срока годности у неё НЕТ ПО ЗАМЫСЛУ. Вставка сюда не пишет (пол авторства —
`created_at` строки), пишет только UPDATE и только по колонкам, которые вправду
изменились, а удаление строки уносит её авторство целиком. То есть таблица
растёт не от потока синхронизации, а от РУЧНЫХ правок: строка, заведённая и
больше не тронутая, не стоит в ней ничего, а пациент, которому за годы
исправили телефон, адрес и заметку, стоит трёх строк — не трёх на правку.
Чистить её нельзя ровно по той причине, ради которой она заведена: вычищенное
авторство — это потерянный спор за колонку (C-1).
...
>   **Сделано в 0.7.0 — см. «Одна лаборатория на всю клинику» ниже.**
...
## Одна лаборатория на всю клинику (сделано, 0.7.0)

Это та самая «маршрутизация лаборатории», которая стояла выше как отдельная
задача после Фазы 2. Владелец сформулировал её проще, чем спецификация: «the lab
can be one single so whenever clinic registrates the patient the all clinics
laborants should see the list of the patients and enter the data.» Лаборатория —
служба КЛИНИКИ, а не здания: пробирку из любого филиала берёт в работу тот
лаборант, у кого стоит анализатор.

**Настройка, а не константа.** У клиники с двумя настоящими лабораториями (в
каждом корпусе своя) общая очередь означала бы, что Юнусабад видит пробирки
Чиланзара, — ровно то, от чего Фаза 2 закрывалась фильтром. Поэтому
`doc_settings.lab_scope` (миграция 085) с двумя значениями:

| значение | что видит лаборант |
|---|---|
| `clinic` (по умолчанию) | очередь, ввод результатов и история «Готово» — заказы ВСЕХ филиалов, каждая карточка подписана меткой «Филиал X» |
...
не меняет ничего вовсе: чужих строк (`sync_origin <> NULL`) там нет, и запрос
без фильтра возвращает ровно то же, что запрос с фильтром.

**Настройка ЕДЕТ со справочником.** `lab_scope` добавлен в
`DOC_SETTINGS_COLUMNS` (`branch-sync/catalogue.js`) и путешествует той же
порцией, что и прайс: решение принимает главный филиал, остальные соглашаются.
Если бы каждое здание решало у себя, филиал ждал бы, что его пробирки возьмёт в
работу главный корпус, а главный корпус их бы не видел — и никто бы не узнал,
потому что молчащая очередь выглядит как «сегодня нет заказов».

Заодно закрыта обратная сторона разных версий на двух концах: `applyCatalogue`
теперь пропускает колонку `doc_settings`, которой у ПОЛУЧАТЕЛЯ ещё нет. Первая
такая колонка — `lab_scope`; без пропуска обновлённый главный филиал ронял бы
приём справочника у необновлённого филиала на «no such column», а приём идёт
одной транзакцией — вместе с настройкой перестал бы доезжать и прайс.

**Границу накладывает одна функция.** `views/lab-scope.js` `scopeQuery()` —
чистый модуль; в `laboratory.js` не осталось ни одного `.is('sync_origin', null)`,
вписанного в запрос руками, и оба запроса ОДНОГО списка (очередь и вкладка
«Готово») идут через неё. Фильтр применяется К ЗАПРОСУ, а не к полученным
строкам: у очереди есть `.limit(5000)`, и отсев после выборки означал бы, что
лимит съеден чужими строками, а своя работа не доехала.

**Результат едет обратно сам.** `lab_results` входит в `SHIPPED`
(`branch-sync/journal.js`), а `visit_services.status` — тоже; лаборант вводит
результат на приехавшей строке, и он возвращается в то здание, где пациента
регистрировали. Ничего специального для этого делать не пришлось.

**Чего эта задача НЕ трогает** (решение владельца 2026-09-02 в силе):
`doctor-room.js`, `visits.js`, `requests-inbox.js`, `procedures.js`,
`consultation.js` и очередь регистратуры остаются пофилиальными. Это закреплено
тестом (`views/lab-scope.test.js`), а не только словами.

**Файлы:** `server/db/migrations/085_lab_scope.sql`,
`server/db/schema-registry.js`, `server/services/branch-sync/catalogue.js`,
`public/js/admin/views/lab-scope.js` (новый), `views/laboratory.js`,
`views/lab-grouping.js`, `public/js/admin/i18n-strings.js`.

**Тесты:** `085.test.js`, `views/lab-scope.test.js`, `views/lab-grouping.test.js`,
`branch-sync/catalogue.test.js`.
