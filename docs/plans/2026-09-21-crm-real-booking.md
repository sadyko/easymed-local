...
## Что построено (сервер, 2026-09-21)

- **Ссылка живёт на строке.** `crm_request_services.visit_id` (миграция 142, `ON DELETE SET NULL` — удаление визита гасит ссылку, а не запирает визит; 143 пересобирает таблицу на машинах, где 142 накатилась ещё с запретом). Филиалам не едет: таблицы нет в `SHIPPED`.
- **Запись = «Записан».** `ensure_visit` (все три пути) через `settleCrmOnBooking`: свободные строки этого дня (дата равна дню, не «не позже») берут `visit_id`, остаются `pending`; родитель едет в `scheduledStageKey()` только вперёд и только из живых ступеней; лид без строк едет сам; строка на мёртвом визите (`cancelled`/`no_show`) записывается заново. Роли `callcenter` в `ENSURE_ROLES` и `BOOK_ROLES`.
- **«Пришёл» = событие, не запись.** `crm/visit-status.js`: `crmVisitStatus` (arrived → строки `done`, родитель → won только когда ждать нечего, иначе «Записан» + ближайшая дата; no_show → сидовая «Не пришёл» из живых; cancelled → строки освобождаются, родитель откатывается из «Записан», если занятых дней не осталось) плюс проход по заявкам без строк того же пациента и дня.
- **Доказательства прихода** — потому что кнопку «Пришёл» в клинике не нажимает никто: платёж (`record_payment`, `record_payment_split`), долг у кассы (`mark_invoice_debt`), счёт по акту контрагенту (`create_invoice_for_visit` с `payer_id`), работа над услугой в `collected/in_progress/resulted/completed` (`save_lab_results`, `procedure_complete`, LIS-приём, `/api/db` insert+update по `visit_services`) и всё то же, приехавшее обменом (`branch-sync/records.js`). Сторож в `crmVisitEvidence`/`crmVisitStatus`: отменённый, не пришедший и **будущий** (местный день > сегодня) визит не доказывает ничего; статус самого визита при этом не меняется.
- **Визит дня уже есть.** Пустой (ни услуг, ни счёта) — переносится тем же `calendar_book`: `{created:false, booked:true, moved:true, from:{start:'HH:MM', doctor_id, doctor_name}}`. С работой — `{booked:false, reason:'day_visit_busy', day_visit:{id, visit_date, start:'HH:MM', duration_minutes, doctor_id, doctor_name}}`; проверка слота до первой записи как была.
- **Словарь.** `visits.status` — пять слов миграции 003 (`scheduled/confirmed/arrived/cancelled/no_show`); приход ровно один — `arrived`. `visit_services.status` — машина из 041; доказательные — четыре последних.
